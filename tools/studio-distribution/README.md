...
The same logical-path fixture is enforced independently by Package Artifact verification/publication, canonical Distribution assembly, and installed-generation health verification. A self-consistent legacy manifest or receipt is not an exemption. The downstream Python implementations are repository reference oracles only; the formal Editor, Launcher, Installer, and Repair executables must consume the portable contracts from C#/.NET or native code and must not start or ship repository Python.

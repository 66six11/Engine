...
| Vulkan buffer barrier refpage: https://docs.vulkan.org/refpages/latest/refpages/source/VkBufferMemoryBarrier2.html | buffer barrier 表达 buffer range 的内存依赖，stage/access 仍需匹配真实 producer / consumer。 | `rhi_vulkan_rendergraph` 先把 RG buffer `TransferWrite`、`ShaderRead(fragment/compute)` 与 `StorageReadWrite(compute)` 映射为 `VkBufferMemoryBarrier2` 字段；offset/size 由录制侧按实际绑定范围提供。 |
...
| 19 Compute dispatch baseline | 19.1 已补 `Dispatch` command summary、`builtin.compute-dispatch` schema 和 compile-only smoke；后续再补 compute capability、compute pipeline wrapper、storage descriptor 绑定和真实 `vkCmdDispatch` smoke | 约束 compute 仍走显式 resource access，不把任意 compute callback 当默认路径 |
...
- Storage image/buffer read-write 必须是明确 access，不能复用模糊的 texture read + color write 组合；buffer 侧第一版已提供
  `StorageReadWrite(compute)`，并在 `--smoke-rendergraph` 中验证 `TransferWrite -> StorageReadWrite(compute)` 的
  `VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT` 与 storage read/write access 映射。
...
- Compute pass 第一版仍使用 typed schema、params 和 command summary；`Dispatch` 命令只允许出现在
  `builtin.compute-dispatch` 一类执行模型中。当前已完成 compile-only 语义和 schema smoke，尚未创建真实
  compute pipeline 或录制 `vkCmdDispatch`。

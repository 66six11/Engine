...
| 18 RenderGraph buffer / storage / MRT | 18.1 已落地 buffer handle、desc、lifetime、access、import/transient buffer、read/write slots、dependency/lifetime/final transition diagnostics；18.2 已补 buffer barrier 映射；18.3 已补 MRT named color slots schema 和真实 dynamic rendering multi-color clear smoke；后续补 storage read/write 和 pipeline desc 多 color format | 先稳定后端无关 resource/access/state，再进入 `rhi_vulkan_rendergraph` 映射要求 |

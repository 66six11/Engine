...
`ValidateStudioViewportNativeRuntimeContract` 对最终 `$(TargetDir)` sibling 执行完整 V9 required / legacy V1--V8 forbidden
admission。这个 gate 不允许 V1--V8 fallback，也不从其他 `bin/`、test preset 或历史隔离输出发现“最新”DLL；失败时必须重建显式选择的
...
`editor_viewport_open_stream_v9_for_test` 是允许的测试扩展；正式 Release Editor Image 必须拒绝该 export。
...
保留 typed context，以及同帧 per-view Solid/Wireframe 不互相污染。`fillModeNonSolid` 未启用时，V9 submit 必须在
...
round-trip 后不出现 runtime EntityId、product generation/hash、Basic resource/material key 或 GPU handle。若同时改动 V9
viewport producer/native ABI，必须证明 malformed V9 packet 被整帧拒绝；item-level asset binding failure 不能被提升为
...
涉及 V9 scene packet、`BasicRenderViewSceneDesc::sourceRevision` 或 Frame Debug projection 时，`--smoke-editor-frame-debugger`
必须断言 capture JSON、panel 与冻结的 RenderView diagnostics 精确记录同一个 `sourceRevision`；旧 ImGui viewport 未绑定
authoritative SceneDocument 时允许为 `0`。Studio V9 scene-mesh process acceptance 必须另行断言 request、receipt 与实际 scene
...
涉及 Studio Transform Gizmo、`GizmoChanged`、`GizmoModeChanged`、V9 Gizmo packet 或 renderer gizmo lines 时，还必须证明：
轴/环命中和近平行退化数学为 UI-neutral；preview sample 不推进 `MinimumPresentableSequence`；240 次连续 preview 仍允许已完成帧呈现；
release 恰好一次 `SetEntityTransformAsync`，no-op/Escape/capture/focus/mode/stale/failure 不提交或回滚；V9 native smoke 拒绝
invalid kind/axis、孤立 flag/payload 和非零未标记 payload，并以 `lastDebugWorldLineCount` 证明匹配 proxy 轴被 renderer-owned
平移轴或三组 64 段旋转环替换。最小 focused gate：

```powershell
dotnet test apps\studio\Tests\Asharia.Studio.Application.Tests\Asharia.Studio.Application.Tests.csproj --filter "ViewportSessionTests|ViewportTranslateGizmoTests|ViewportRotateGizmoTests"

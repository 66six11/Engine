...
[Activation Eligibility v1](../architecture/adr-activation-eligibility-v1.md)、
...
`RegistryExpired` 与旧 generation fail-closed。registration-only generated Host 的五个 callbacks 与 payload accessors 仍必须是
abort/counter probes；该路径不得因 ProcessScope 实现而开始执行 lifecycle 或 accessor。

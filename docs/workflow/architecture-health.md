...
| 无兼容硬切 | Studio legacy composition、同步 async lifecycle、editor-side closure transaction、native viewport V1–V8 raw-token compatibility contract | V1–V8 frame exports 已硬切删除；不得为 V9 production consumer 恢复旧入口或兼容层 |
...
- Vulkan context、producer、outstanding/retiring packet 与析构只由 RenderThread 访问。V9 stream 请求复制 owning
...
结论：owner thread、V9 stream lifecycle 与 bounded mailbox 基线已经成立；下一次 contract hard-cut 目标是显式
`NativeSessionHandle + epoch + bounded lease`。查询继续保持只读 snapshot；V1–V8 frame exports 已删除，

...
```powershell
cmd /c "build\conan\msvc-debug\Debug\generators\conanbuild.bat && cmake -S packages\project-core -B build\cmake\package-project-core-tests-msvc-debug -G Ninja -DCMAKE_BUILD_TYPE=Debug -DASHARIA_BUILD_TESTS=ON -DCMAKE_TOOLCHAIN_FILE=%CD%/build/conan/msvc-debug/Debug/generators/conan_toolchain.cmake && cmake --build build\cmake\package-project-core-tests-msvc-debug && ctest --test-dir build\cmake\package-project-core-tests-msvc-debug --output-on-failure"
```
...
- This does not make a general binary SDK or stable C++ ABI. `asharia::scene_native` is one narrow,
  ...
  validated local Transform and UTF-8 display/debug name values; it does not make the package's C++
  API binary-stable or expose hierarchy/world transforms, change notifications, or general
  components.

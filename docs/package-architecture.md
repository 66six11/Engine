...
每个 package 后续都应该有一个 manifest，建议命名为 `Asharia.package.json`：
...
其中 `packages/rhi-vulkan -> packages/rendergraph` 只适用于 RenderGraph/Vulkan 适配 target；基础 `asharia::rhi_vulkan` target 不应公开依赖 RenderGraph。

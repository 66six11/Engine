...
- Scene View camera、overlay 和 debug line intent 已能进入 `RenderView` diagnostics；存在
  ...
  selection outline 继续推进前，仍必须补 external manifest loader、更完整 grid settings UI、热更新和更完整
  provider contract；camera-aware grid visibility、world-grid/source overlay diagnostics、Scene View `sceneGrid`
  settings bridge、grid color UI v0、built-in contribution default 与高视角 readback 已有最小闭环。
...
- This is not the final provider architecture: Scene View grid spacing/fade/opacity/color now has an editor settings to
  ...
  bootstrap. There is still no external manifest loader, hot reload, or complete settings UI yet. Camera-aware grid

...
6. **Asset-core + asset-pipeline + resource upload baseline**：`asset-core` 建立 GUID、source metadata、handle 和产品 key 数据模型；`asset-pipeline` 先落地显式 source/.ameta metadata discovery、diagnostics、source file snapshot/hash 和 product manifest IO baseline；后续 `asset-processor` / pipeline slice 再负责 source scan、import 调度、cache hit/miss planning 和 dependency invalidation；renderer/RHI 只接 mesh/texture upload 请求与已解析 product data。
...
| 22 | Asset-core + asset-pipeline + resource upload baseline | `asset-core` 拥有 GUID、source metadata、import settings、product key/cache key 和 dependency 数据模型；`.ameta` deterministic metadata IO 已落在可选 `asharia::asset_core_io` target，`asset_core` identity/catalog 仍只依赖 core；canonical `sourcePath` 合同已收敛为 project-relative generic path；`asset-pipeline` 已落地显式 source/.ameta metadata discovery baseline、显式 source file snapshot/hash baseline 和 product manifest IO baseline，后续再推进 source scan、import 调度、cache hit/miss planning 和 dependency invalidation；renderer/RHI 只消费 resource handle、product data 和 upload request，补 staging/linear upload allocator | `asharia-asset-core-smoke-tests` 已覆盖 `.ameta` round-trip / malformed input / settings hash mismatch / 非规范 sourcePath；`asharia-asset-pipeline-smoke-tests` 覆盖 discovery round-trip、source snapshot determinism/content change、product manifest IO round-trip/malformed/duplicate/mismatch negative paths、missing/malformed metadata、source path mismatch、duplicate GUID/path、entry/metadata 非规范 sourcePath、缺失/非普通 source file 和 duplicate snapshot source path；后续补 `--smoke-mesh-resource`、`--smoke-texture-upload`；runtime 不直接依赖 source asset 路径；editor 文件变化不直接落在 UI 或 `asset-core` |

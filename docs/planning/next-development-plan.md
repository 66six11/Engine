...
| D. Graph-visible GPU work | clear、fill、upload、copy、barrier 不能长期藏在 graph 外；外部 pre-pass 必须显式进入 diagnostics | compute dispatch 录制前有 `vkCmdFillBuffer`，再以 imported initial `TransferWrite` 交给 RenderGraph；这会削弱 graph 对真实 GPU work 的可观测性 | fill/clear/upload 进入 RenderGraph pass/command，或作为 named external pre-pass 在 diagnostics、Frame Debug 和 review 输出中可见；新增 smoke 验证 execution event 顺序 |

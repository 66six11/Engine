...
| Material | 已有 CPU-only signature、descriptor contract、pipeline key hash smoke、renderer binding smoke、shader reflection adapter、CPU-only `.ashader` parser/document diagnostics、generated Slang skeleton；#150 正在验证 generated Slang compile/reflection smoke | material asset IO/editor path、`.amat` 最小 IO、asset cook 和 renderer material product 消费 |
...
1. `#150 [Slice] Materials: validate generated Slang compile/reflection smoke`：继续 shader/material MVP Milestone 3，
   验证 generated Slang source 能通过 Slang compile、`spirv-val`、reflection JSON 和 reflection-to-material-signature
   smoke，不做 `.amat`、asset cook 或 editor UI。

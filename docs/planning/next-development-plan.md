...
| B. RenderView real input contract | camera、overlay、debug line、frame params 不能只进入 diagnostics；需要 renderer-owned per-view constants / pass input 和可见 debug-line/grid pass | B.1 已落地：Scene View request 携带 camera 与 overlay intent，`EditorViewportOverlayProvider` 生成 `BasicDebugWorldLine`，overlay enabled 的 `recordViewFrame()` 会插入 `builtin.render-view-overlay` pass，把 camera/frame/line-count 作为 typed params 与 command summary 记录，并触碰目标 attachment；当前仍未实际绘制 world line | 下一步补 camera-aware policy 和可见 debug-line/grid renderer pass；smoke 必须继续验证 Scene View graph 含 overlay/input pass，Game/Preview 不接收 Scene-only pass |
| C. Multi-view request model | viewport request 不能是单个 optional；Scene/Game/Preview/多面板同帧需要 per-view id、结果、diagnostics 和 refresh reason | `EditorViewportCoordinator::requestViewport()` 当前覆盖 `requestedViewport_`；只记录一个 requested view | coordinator 收集 keyed request list；同帧多 RenderView 有独立 texture result、diagnostics snapshot、execution events 和 failure context；现有单 Scene View smoke 不退化 |

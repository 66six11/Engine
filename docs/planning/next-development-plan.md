...
| E. Editor state and command model | panel、context、app bootstrap 不能继续膨胀为 service locator / god object；持久 mutation 需要 command/transaction 或 capability-scoped context | `EditorFrameContext` 已从扁平服务集合收敛为 `ui` / `diagnostics` / `settings` / `tools` / `input` / `renderGraph` / `viewport` capability groups；action dispatch 使用 `EditorActionInvokeContext` 发事件，callback 只接收 `EditorActionContext`，不再接收完整 `EditorContext`；panel 窗口准备阶段已改用只暴露 `ui` 的 `EditorPanelWindowContext`；panel `draw()` 虚接口已改用由 registry 组装的 `EditorPanelDrawContext`，顶层 `EditorFrameContext` 只停留在 `EditorPanelRegistry::drawPanels()` 适配边界；当前内置 panel 的 `draw()` 实现会继续映射到 panel-local context，helper 不再传播完整 frame context；`editor_app.cpp` 仍同时承担 smoke、frame loop、ImGui/Vulkan glue 等职责 | 继续把 `EditorPanelDrawContext` 从全 panel 能力合集拆成更窄的 category/per-panel context；继续拆出 app bootstrap/registration、smoke checks、ImGui Vulkan frame renderer；新增或更新 editor smoke 覆盖 action、event、panel 与 viewport 状态 |

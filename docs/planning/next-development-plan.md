...
- `asharia-editor` 已接入 Dear ImGui host、dockspace/menu、panel/action/event registry、
  Scene View sampled viewport、viewport overlay flags、ImGui texture registry、`--smoke-editor-shell`、
  `--smoke-editor-viewport` 和 `--smoke-editor-viewport-resize`。
...
| 21 | Gizmo / grid / debug draw | 21.1 已在 editor viewport request/result、coordinator 和 ImGui texture metadata 中加入 Scene/debug viewport flags，并在 coordinator 层清空 Game/Preview 的 Scene-only authoring flags；Game View 可保留显式 debug overlay/debug gizmo intent。后续 21.2 从 grid pass 开始把这些 flags 转成 renderer-side pass 输入。renderer 只实现对应 draw packet / overlay pass 消费 | `--smoke-editor-viewport` 验证 Scene View flag defaults、Scene flags 保留、Game 清空 Scene-only authoring flags 但保留 explicit debug flags、Preview effective flags 为空和至少一帧 flagged Scene View render；后续 Scene View graph 可包含 editor-only authoring pass，Game View graph 只包含显式 opt-in 的 debug pass |

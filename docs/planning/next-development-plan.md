...
| E. Editor state and command model | panel、context、app bootstrap 不能继续膨胀为 service locator / god object；持久 mutation 需要 command/transaction 或 capability-scoped context | `EditorFrameContext` 已从扁平服务集合收敛为 `ui` / `diagnostics` / `settings` / `tools` / `input` / `renderGraph` / `viewport` capability groups；action dispatch 使用 `EditorActionInvokeContext` 发事件，callback 只接收 `EditorActionContext`，不再接收完整 `EditorContext`；panel 窗口准备阶段已改用只暴露 `ui` 的 `EditorPanelWindowContext`；`editor_app.cpp` 仍同时承担 smoke、frame loop、ImGui/Vulkan glue 等职责，panel `draw()` 虚接口仍以顶层 `EditorFrameContext` 作为适配入口 | 继续把 panel `draw()` API 向逐面板窄 context 推进；拆出 app bootstrap/registration、smoke checks、ImGui Vulkan frame renderer；新增或更新 editor smoke 覆盖 action、event、panel 与 viewport 状态 |

...
| E. Editor state and command model | panel、context、app bootstrap 不能继续膨胀为 service locator / god object；持久 mutation 需要 command/transaction 或 capability-scoped context | `EditorFrameContext` / `EditorContext` 暴露宽服务集合；`editor_app.cpp` 同时承担注册、smoke、frame loop、ImGui/Vulkan glue 等职责 | 先定义 editor command/transaction 边界和 capability contexts；拆出 app bootstrap/registration、smoke checks、ImGui Vulkan frame renderer；新增或更新 editor smoke 覆盖 action、event、panel 与 viewport 状态 |
| F. RenderGraph API / implementation split | 大型 header-only public API 在继续扩展 cache、alias、multi-queue 或 unsafe/native pass 前需要拆分策略 | `packages/rendergraph` 当前是 INTERFACE target，`render_graph.hpp` 同时承载声明、compile、diagnostics formatting 和实现细节 | 在新增复杂 graph feature 前写 ADR 或拆分计划；至少把 public builder/compiled graph/diagnostics API 与实现细节的稳定边界列清，并用 package test 锁住 public contract |

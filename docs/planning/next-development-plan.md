...
| F. RenderGraph API / implementation split | 大型 public API 在继续扩展 cache、alias、multi-queue 或 unsafe/native pass 前需要拆分策略 | `packages/rendergraph` 已改为 STATIC target；types、command list、execution context/registry、builder、compile result 和 diagnostics 已拆成窄公共头，`render_graph.hpp` 现在是聚合兼容头；command list、registry、非模板 `PassBuilder`、resource/pass facade、public compile/execute overload 和 diagnostics formatting 已进入 `src/`；不访问 graph 私有状态的 compile/dependency/schema helper 已从 builder header 收敛到 `.cpp` 文件局部函数 | 在新增复杂 graph feature 前继续评估剩余 validation/dependency/lifetime/debug helper 声明是否需要内部 implementation header 或数据/pimpl 边界，并保持 package header tests 锁住 public contract |

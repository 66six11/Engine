...
性能诊断底座和未来编辑器性能面板的技术细节见 `docs/systems/performance-profiling.md`；编辑器从通用
RenderTarget / RenderView / ImGui sampled texture registration 契约稳定后以 host 和消费者身份接入，
不拥有 renderer 或 Vulkan 后端对象。Editor host、ImGui shell、panel/action/event、viewport texture
registry、输入路由和 editor 子阶段拆分见独立开发文档 `docs/planning/editor-development-plan.md`。
...
| 17 | Editor viewport host | 17.1 已在 `apps/editor/src/editor_viewport.*` 增加 backend-neutral viewport request/result model；Scene View panel 只提交 `EditorViewportRequest` 并绘制上一帧完成的 texture，host 在 panel draw 外把 `EditorExtent2D` 转成 Vulkan extent 并调用 `recordViewFrame()`。后续继续拆 `ImGuiTextureRegistry` 管理 descriptor 注册和 completed-frame 延迟退役。详细切分见 `docs/planning/editor-development-plan.md` 的 17.x | editor viewport 能显示通用 RT 输出；descriptor 注册/注销由 editor integration 管理并延迟释放；旧 viewport RT 通过 frame deferred destroy 释放；`--smoke-editor-viewport` / resize smoke 验证至少一次 viewport texture 呈现和 resize 后继续呈现 |

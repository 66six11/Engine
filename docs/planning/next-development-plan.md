...
- native V8 request 与真实 Vulkan多stream/持久slot、Scene/Game/Preview smoke。
...
与单个 ProjectEditId 写一次 ProjectSession；Escape/capture/focus/stale revision/failure 均取消或回滚。V8 只增加 typed Gizmo packet，
renderer 复用已有 debug world-line route，不新增 Vulkan resource/pass/sync。rotate/scale/plane/local/snap、多选、Physics、通用工具 registry、
...
- Wireframe 只在 logical device 已启用 optional `fillModeNonSolid` 时使用 `VK_POLYGON_MODE_LINE`；不可用时 V8
...
- missing/wrong-kind/stale/invalid binding 逐 item no-draw，并保留 scene object/asset/revision context；空 scene 不生成
  scene-mesh pass。malformed V8 packet 必须拒绝整帧，不提交部分 draw。

...
| F. RenderGraph API / implementation split | 大型 public API 在继续扩展 cache、alias、multi-queue 或 unsafe/native pass 前需要拆分策略 | `packages/rendergraph` 已改为 STATIC target，`RenderGraphCommandList`、registry、非模板 `PassBuilder`、resource/pass facade、public compile/execute overload 和 diagnostics formatting 已进入 `src/render_graph.cpp`；`render_graph.hpp` 仍承载 compile/validation/dependency/lifetime 核心实现 | 在新增复杂 graph feature 前继续迁移 compile/validation 实现；至少把 public builder/compiled graph/diagnostics API 与实现细节的稳定边界列清，并用 package test 锁住 public contract |

...
| 19 | Compute dispatch baseline | Stage 19.1 已补 `Dispatch` command summary 与 `builtin.compute-dispatch` schema；后续在设备选择中记录 compute capability，再新增 compute pipeline wrapper、descriptor/storage buffer 绑定和 `vkCmdDispatch` 真实 smoke | 当前由 `--smoke-rendergraph` 覆盖 compile-only compute dispatch 语义；后续新增 `--smoke-compute-dispatch` 并要求 sync validation 无 error/warning |

...
| 17 | Editor viewport host | 在 `apps/editor` 接入 `asharia::renderer_basic_vulkan`，editor host 以 Scene View 面板尺寸驱动 sampled render target，使用既有 `recordViewFrame()` 写入 target，并通过 ImGui Vulkan backend 注册 descriptor 后显示；resize 时保留上一张 presented texture 并在后台准备 pending texture，dock chrome 仍只走 ImGui swapchain pass | editor viewport 能显示通用 RT 输出；descriptor 注册/注销由 editor integration 管理，旧 viewport RT 通过 frame deferred destroy 释放，teardown 先等 queue idle 再释放 ImGui texture；`--smoke-editor-shell`/`--smoke-editor-viewport` 验证至少一次 viewport texture 呈现 |
| 18 | RenderGraph buffer / storage / MRT | Stage 18.1 已落地后端无关 buffer handle、buffer desc、buffer access、import/transient buffer、buffer read/write slot、dependency sort、lifetime/final transition 计划和 debug table；Stage 18.2 已补 buffer usage / transition / `VkBufferMemoryBarrier2` adapter 映射；后续子阶段再补 storage read/write、MRT color slots 和 dynamic rendering pipeline desc 多 color format | `--smoke-rendergraph` 覆盖 buffer dependency、imported final state、missing producer、missing shader stage、zero-size buffer 和 buffer Vulkan adapter 字段；后续新增 storage/MRT smoke 与 `--smoke-mrt` |
...
| 22 | Asset-core + asset-pipeline + resource upload baseline | 新增 GUID、source path、import settings、product hash/cache；`asset-pipeline` / `asset-processor` 后续负责文件修改监听、source hash、metadata IO、import 调度、product cache manifest 和 dependency invalidation；最小 mesh/texture resource handle 和 staging/linear upload allocator | `--smoke-mesh-resource`、`--smoke-texture-upload`；runtime 不直接依赖 source asset 路径；editor 文件修改更新不直接落在 UI 或 `asset-core` |

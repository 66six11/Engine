...
- Grid spacing、major spacing、optional fade、opacity 和 color 已接入 editor user settings，并通过 Scene View request 进入
  ...
  bootstrap 用它处理新 settings 和旧 settings 缺省迁移；Editor Settings 已有左侧目录/右侧内容的 grid settings v0；后续再补 external manifest loader 和热更新。
...
2. **Grid 仍需 external manifest / hot reload 化**: renderer-owned fullscreen world-grid pass 已落地，grid 不再依赖固定原点 debug-line packet，已补只按垂直距离计算的 camera-height 1/2/5/10 spacing LOD、低高度远水平距离仍保持 base LOD、默认无距离淡出和高视角 readback 验证；world-grid desc/source overlay id diagnostics 已进入 RenderView/Frame Debug smoke；spacing、major spacing、optional fade、opacity 和 color 已通过 `EditorSettings::sceneGrid` 接入 Scene View request 到 RenderView 的数据路径；Scene grid overlay contribution 已声明 built-in 默认值，settings bootstrap 使用同一默认值，Editor Settings 已有左侧目录/右侧内容的 grid settings v0。尚未把 external manifest loader 或热更新接入。

...
**待完成**:
- Pixel/readback 级 GPU smoke 验证不同 camera position 产生不同渲染输出；当前 smoke 覆盖 graph pass、draw event 和 Vulkan validation。
...
2. **Grid 仍需 pixel smoke 和设置化**: renderer-owned fullscreen world-grid pass 已落地，grid 不再依赖固定原点 debug-line packet；尚未做像素/readback 级 camera-difference 验证，也未把 spacing/fade/color 接入 manifest-backed settings。

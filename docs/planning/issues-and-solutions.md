...
2. **Grid 仍需设置化**: renderer-owned fullscreen world-grid pass 已落地，grid 不再依赖固定原点 debug-line packet，已补只按垂直距离计算的 camera-height 1/2/5/10 spacing LOD、低高度远水平距离仍保持 base LOD、默认无距离淡出和高视角 readback 验证；world-grid desc/source overlay id diagnostics 已进入 RenderView/Frame Debug smoke。尚未把 spacing/color/optional fade 接入 manifest-backed settings。

...
**验证与后续**:
- `--smoke-render-view-grid-readback` 已补两组 camera 的 offscreen RenderView -> transfer readback 验证，覆盖 Scene View grid diagnostics、`builtin.render-view-world-grid` pass、`DrawWorldGrid` execution event、像素 spread、camera-difference 和 Vulkan present/layout validation。
- 后续可把 grid spacing/fade/color 接入 manifest-backed settings，并补 source overlay id diagnostics。
...
2. **Grid 仍需设置化和 source 诊断**: renderer-owned fullscreen world-grid pass 已落地，grid 不再依赖固定原点 debug-line packet，已补 screen-footprint 自适应 spacing，并由 `--smoke-render-view-grid-readback` 覆盖像素/readback 级 camera-difference 验证；尚未把 spacing/fade/color 接入 manifest-backed settings，也未补 source overlay id diagnostics。
...
短期 (2-4 轮迭代):
  Grid manifest-backed settings + source overlay id diagnostics (1.2)

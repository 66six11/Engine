...
- `EditorContext` 曾暴露 panel registry、event queue、diagnostics、frame debugger、i18n、settings、workspace 和 tools；
  当前已移除 panel registry、frame debugger 和 action invoke facade，剩余 event queue、diagnostics、i18n、settings、
  workspace 和 tools 仍需继续收敛
...
**待完成 (Step 2b)**:
- 继续拆分 `EditorContext` 中 workspace/settings/event/diagnostics/tools 等剩余宽 facade
...
- **编辑器层**: `EditorContext` — 当前剩余 event queue、diagnostics、i18n、settings、workspace 和 tools；
  action invoke 已拆到 `EditorActionServices`
- **评价**:
...
| **Service Locator lite (轻)** | `EditorContext` — 已从 8 个 subsystem 降到 6 个 app service，action/panel/frame-debug 能力已拆出，但剩余 workspace/settings/event/diagnostics/tools 仍是宽 facade | **P3** | Unreal `UWorld` 暴露 GetWorld() 访问几乎所有子系统——但 Unreal 有 module 边界限制，你的 context 当前没有。Unity `EditorApplication` 作为 static facade 也是宽接口 |
...
**反模式截止线**:
- `EditorContext` 在 6 个以上 subsystem 后继续拆 (当前 6 个；action/panel/frame-debug 已拆出)

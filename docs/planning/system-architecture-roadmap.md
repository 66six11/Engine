...
<https://docs.o3de.org/docs/user-guide/programming/memory-management/>、
<https://docs.o3de.org/docs/user-guide/programming/memory/allocators/>、
...
<https://dev.epicgames.com/documentation/en-us/unreal-engine/crash-reporting-in-unreal-engine>
...
| [Host Executable Binding Receipt](../architecture/adr-host-executable-binding-receipt-v1.md) | 构建/链接后把 Blueprint/composition、same-index configured target/compiler、observed registration snapshot 与 exact staged executable bytes 绑定成 content-addressed generation 的证据 | lifecycle activation、`Ready`/current process state、新的依赖图、builder trust、签名或跨 generation ABI 承诺 |
| [Activation Eligibility](../architecture/adr-activation-eligibility-v1.md) | sealed Session/Blueprint/binding/launch handoff 派生的一次性 pre-registration authority，以及绑定同一次 table instance 的 descriptor authority | Effective Session `Ready`、持久化 receipt、detached boolean、lifecycle success 或 hostile-native sandbox |
...
flowchart LR
    ...
    Binding --> Eligibility
    Launch --> Eligibility
    Eligibility -.->|"future admitted lifecycle input"| Host
```
...
- Effective Session 先将 Distribution、Project Lock 与 Host Profile 对证并报告状态；Host Composition Plan 再固定逻辑
  packages/modules/entries/contributions；build plan 决定编译哪些 targets；Host Activation Blueprint 在构建前固定 scopes、
    ...
    lineage 在 provider recording 前交叉匹配，并将 descriptor authority 绑定到同一次 table instance；该实现已加入 #292，最终门禁
    结果以 Done evidence 为准，production issuer 与 normal Host 接线仍待后续 adapter。Module 默认
...
完整 Platform Support System 在 Kernel primitives 之上发布 immutable lifecycle 与 `PlatformCapabilitiesSnapshot`。该快照只含
OS/architecture、CPU topology、memory class/pressure、display/DPI、window/process/dynamic-library 等平台事实；GPU
features/limits 归 RHI，物理输入设备状态归 Input，不建立跨领域 mutable capability bag。
...
- **拥有：** memory domain/tag registry、CPU/GPU/resource budget projection、pressure snapshot、trim protocol 和 owner result。
- **不拥有：** Content/Renderer/Audio 的 allocator、cache、resource lifetime 或 eviction policy。
- **当前映射：** Kernel 只有最小 allocator contract；统一 domain/budget/pressure owner 尚未实现。
- **目标：** Kernel 在 package activation 前提供 allocator/tag hook；完整 Memory & Budget System 在 Host activation 后管理预算和压力合同；Observability 只读聚合，Runtime Storage 只管理自己的 IO buffer/cache domain。

### 6.11 Observability & Validation

- **拥有：** structured diagnostics、trace/counter 协议、CPU/GPU profiling adapter、bounded log router、本地 crash evidence、architecture tests、smoke orchestration 和 evidence schema。
- **不拥有：** 被观测系统的控制流或业务状态。
- **当前映射：** `profiling`、package-local tests、sample-viewer smoke、文档和编码检查。
- **目标：** 每个系统公开低耦合的诊断快照；bootstrap log/fatal evidence 连续迁移到 runtime router；架构约束能由 CI 自动检查，而不是只靠评审记忆。

### 6.12 First-party 系统包目录
...
| Memory & Budget | bundled / required by standard profiles | memory domain/tag registry、budget/pressure snapshot、trim protocol、platform/RHI projections、Editor inspector | allocator/tag hook 留在 Kernel；各系统拥有 allocator/cache/trim；Observability 只读 |
...
| Observability | bundled / profile-selected | diagnostics、trace/counters、memory snapshot adapter、CPU/GPU profiling adapters、bounded log、local crash report、automation/screenshot/performance evidence | 只观察公共契约，memory policy/trim/release 仍由 Memory 与领域 owner 执行；Console/CVar 属于 Settings |
...
基础 capability vocabulary 在 Phase 1/1.5 就冻结：mount-relative file read/write、process launch、network、Editor
document mutation、World mutation、console command、native backend access 与 debug-only operation。Manifest 负责声明，Host
Profile 按 role/scope grant 或 deny，Storage/World/Editor/Settings 等实际 owner 的 public facade 执行检查。它不是全局
`SecurityService`；registry signature、远程来源 trust、license、下载完整性和更强 sandbox 仍在 Phase 11 落地。
...
### 6.14 Feature Sets、Project Templates 与 Host Profiles
...
| memory budget/pressure snapshot | Memory & Budget；Platform/RHI/各系统提供 facts | Host、Editor、Observability、system owners | per-domain、只读；trim 由 allocation/resource owner 执行 |
| granted capability token/facade | Package Runtime declaration -> Host grant | system factory、script context、public facade | role/scope-bound、least authority、denial 可诊断、不可升级权限 |
...
| Resolution | 其他完整 installable packages 的 named required/optional dependencies、conflicts；provider `provides/requires` capability metadata 仅预留 |
...
| Activation | logical modules、host role、loading phase、system factories、ordered dependencies、link/load mode、required execution capabilities |
...
manifest 的 execution capability declaration 在 v0 即参与 Host role/scope grant/deny；它只控制已解析 module 能做什么，不替代 package dependency。`provides/requires` provider capability metadata 可以预留给搜索和诊断，但 v0 不用它满足依赖。只有出现第二个真实可用实现、具名替换已经造成实际维护问题，并完成 provider-resolution ADR 与冲突测试后，才引入 `requires capability` 和 exclusive provider 求解。
...
| `packages/systems/observability` | diagnostics/log router、trace/counter contracts、profiling/memory adapters、local crash evidence、automation/test support | `profiling` 与分散测试支持 | 完整 bundled/profile-selected Foundation System；不拥有被观测系统状态 |
...
sequenceDiagram
    ...
    participant Memory as Memory & Budget
    ...
    Host->>Memory: publish final live-domain snapshot
    Host->>Observe: flush bounded diagnostics
    Host->>Memory: close domains
...
- 维持现有 package 和 target 的 machine-readable inventory 与 CI drift gate；
- 已标注每个 target 的 primary owner、module role 和 planned ownership root；后续按真实调用图补 public consumers 与 optional dependency，不用空字段猜测；
...
- `python tools/check_package_topology.py` 通过，当前 source boundaries 均不可选择且不进入 catalog；
...
- 建立 `Minimal`、`Editor`、`Runtime`、`DedicatedServer`、`AssetWorker` Host Profiles，以及 versioned Standard3D/EditorAuthoring/DedicatedServer Feature Sets；五种 Host Profile 的 v1 schema/固定策略/纯数据投影基线、Host Composition Plan v1、Source Build Plan v1、Package Product & Artifact Evidence v1、Artifact Collection/Publication v1、Engine Distribution Manifest v1 / `EngineGenerationId`、Project Lock v2 硬切、Effective Session v1、Distribution Assembly v1、Installed Distribution Repair Verifier v1、Package Factory Declaration v1 与 Host Activation Blueprint v1 已落地；active provider bindings/Binding Plan 与 provider API 已硬切 v2，generated root 只生成 Composition renderer 3/provider v2，Host Template 只生成 renderer 2，#291 已把 active registrar 硬切为 descriptor registration，并由 Static Factory Callback Table v1 拥有 callbacks 与 canonical identity-only snapshot。Host build、binding assembly 与 deep verifier 只接受当前 T2/C3/provider-v2 组合；bindings/Binding Plan v1 与 pre-current renderer/provider tuple 没有 reader/adapter，Host Executable Binding Receipt v1 与 RegistrationSnapshot v1 保持 active evidence schema v1。[Activation Eligibility v1](../architecture/adr-activation-eligibility-v1.md) 已添加 C++ validator、linear wrappers 与 focused tests，最终门禁结果以 #292 Done evidence 为准；production profiles、repair executor/启动 adapter、production launch issuer/normal Host 接线与 #293 ProcessScope lifecycle 仍待后续 Slice；
...
- 冻结 Kernel clock、Host frame/application phases、World simulation clock 与 Tasks delayed job/timer 的所有权；不建立万能 Time System；
...
- 用 synthetic systems、Host Activation Blueprint 和构建后 binding receipt 形成 Minimal/Runtime/Server/Tool headless Host smokes。
...
- Platform capability snapshot 只含平台事实，generation change 可重放；RHI/Input facts 不进入同一个 mutable bag；
...
- memory domain 总量、budget/pressure transition、owner trim result 和 low-memory synthetic event 有测试；Observability 不能释放被观测资源；
- Host phase order、timer completion 与 lifecycle delivery 可记录且确定；fixed/variable simulation clock 由 synthetic World contract 验证；
- denied capability 不创建 OS handle、不修改 owner state，并返回 package/module/scope identity 完整的 structured diagnostic；
...
### Phase 3：Content、Resources 与 Runtime Storage 集成闭环
...
- 只通过 Phase 1.5 已建立的 mount-relative async IO contract 消费 cooked bundle/artifact，不复制 VFS、request queue 或 OS handle owner；
...
- Content streaming smoke 通过 Runtime Storage bundle mount/async/cancel/corrupt result 完成，不直接调用 bootstrap filesystem；
...
- 在 Phase 8 的本地 product pipeline 上增加外部 package trust、registry restore、平台分发 adapter、远程 crash/telemetry 上传和兼容策略；本地 crash capture 已在 Phase 1.5 成立。
...
5. Foundation Services 边界，至少分别记录：Runtime Storage/VFS/async IO 与 bootstrap filesystem；Settings/Device Profile；Tasks baseline；Memory & Budget 与 Kernel allocator/tag hook；PlatformCapabilitiesSnapshot 与 RHI/Input facts；Host phase/time ownership；本地 crash baseline；基础 capability declaration/grant/deny；
...
| Package resolver/lock | deterministic solve、cycle/conflict/missing named dependency/platform/integrity negative tests、Editor/CLI parity；基础 capability declaration/schema validation 前置，外部 trust/resolution tests 后置 |
...
| Platform lifecycle/capabilities | focus/quit/suspend/resume/low-memory/device/display synthetic events、immutable capability generation、RHI/Input fact separation、bounded handler、shutdown while suspended |
...
| Tasks/time baseline | cancel/join/no-leak、Host phase order、timer completion、synthetic fixed/variable clock、shutdown determinism |
| Memory & Budget | bootstrap tag continuity、domain totals、budget/pressure transition、owner trim result、low-memory/OOM evidence、Observability read-only check |
| Observability/crash baseline | early-to-runtime log continuity、bounded local report、build/lock/scope/package generation/callstack/context evidence、pre-activation fatal probe |
| Capability policy baseline | manifest declaration、Host role/scope grant/deny、least-authority factory context、owner state unchanged on denial、structured diagnostic |
...
2. 写 Foundation/Package ADR：Kernel、Host Runtime、scope/lease/registry、manifest/lock、Host Profiles、Platform capability、Memory & Budget、time/update、local crash、capability policy 和状态机；
...
6. 以 Platform capability、Memory & Budget、Runtime Storage、Settings、Tasks/time 和 Observability/local crash baseline 闭合第一个 Foundation Host smoke；

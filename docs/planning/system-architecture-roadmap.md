...
Package Manager 必须提供同一套 headless API 给 Editor、CLI 和 CI：`discover -> solve/reuse -> compose -> plan -> apply -> verify`。`apply` 只能修改 `asharia.packages.json` / `asharia.packages.lock.json`、acquire cache 和生成配置；真实系统实例由 Host 在启动/安全点按可执行 activation plan 创建。

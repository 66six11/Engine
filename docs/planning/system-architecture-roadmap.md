...
8. `asharia.package.json` schema v1 已把当前目录分类为不可选择的 source boundaries；installable v2、Project Manifest v1、Feature Set v2、Package Lockfile v1、五种 Host Profile v1、explicit-source Candidate Discovery v1、deterministic in-memory resolver 与 fail-closed Locked Graph Verification & Reuse v1 已落地，但尚无上游 catalog/index、lock update/apply、生产 `asharia.packages.json` / lock/profile、Build/Activation Plan 和 Editor Package Manager 闭环；

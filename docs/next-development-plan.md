...
- `VulkanFrameLoop` 已接入 `VK_EXT_debug_utils` command label helper 和 label counters；renderer-basic-vulkan
  使用 RenderGraph pass name 标记 GPU command 区间，timestamp query 延迟读回仍是下一步。
...
| 12 | 性能诊断底座 | `packages/profiling`、CPU scope、benchmark CLI 和 RenderGraph compile counters 已接入；GPU timestamp/debug label 只保留后续接口方向 | `--bench-rendergraph` 可在 Release preset 下输出 warmup/frames/p50/p95/max 和 graph counters；`--smoke-*` 行为不变；editor 面板只保留技术约束，不进入当前里程碑 |
...
- editor、asset database、script VM、bindless、async compute 和 transient memory alias 必须保持在暂缓项，直到前置小闭环已经稳定。
...
VkEngine 当前尚未把 editor 作为产品阶段纳入计划，但 RenderGraph 和 profiling 的技术设计不应假设一帧只有一个 RenderGraph。
...
- 编辑器性能面板、EditorHost profiler 和 asset/inspector 性能分析暂不进入当前计划；相关约束只记录在 `performance-profiling-plan.md`。

...
| 5 | RenderGraph transient image | 增加 transient image 声明、Vulkan image/image view/VMA allocation 和 binding 表接入 | 新增 `--smoke-transient`，验证非 backbuffer image transition、lifetime、final shader stage 和 binding |
| 6 | Depth attachment MVP | 增加 dynamic rendering depth attachment，复用已接入的 depth write/read 状态和 binding | 新增 `--smoke-depth-triangle`，validation 无 error/warning |

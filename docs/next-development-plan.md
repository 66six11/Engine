...
| 17 | Editor viewport host | 在 `apps/editor` 接入 `vke::renderer_basic_vulkan`，editor host 以 Scene View 面板尺寸驱动 sampled render target，使用既有 `recordViewFrame()` 写入 target，并通过 ImGui Vulkan backend 注册 descriptor 后显示；dock chrome 仍只走 ImGui swapchain pass | editor viewport 能显示通用 RT 输出；descriptor 注册/注销由 editor integration 管理，teardown 先等 queue idle 再释放 ImGui texture；`--smoke-editor-shell`/`--smoke-editor-viewport` 验证至少一次 viewport texture 呈现 |

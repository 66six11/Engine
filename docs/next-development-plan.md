...
RenderTarget / RenderView / sampled texture bridge 稳定后以 host 和消费者身份接入，不拥有 renderer
或 Vulkan 后端对象。
...
- `--smoke-offscreen-viewport` 已把 editor viewport 的核心离屏路径接入真实 Vulkan 录制路径：持久
  ...
  并通过 renderer 输出 sampled image/view/layout 供未来 editor UI backend 注册显示。
...
本轮审查后的核心调整是：编辑器不再只是远期暂缓项。它应在通用 RenderTarget / RenderView 和 UI sampled
texture bridge 稳定后尽早接入，作为渲染层真实消费方；但 editor 不能成为 renderer 或 RHI 的 owner。
后续路线按“先建立通用渲染目标，再接 editor viewport，再扩 RenderGraph 资源模型，最后进入 material /
lighting / scene”的顺序推进。

1. **通用 RenderTarget / RenderView / sampled texture bridge**：把当前 offscreen viewport 小闭环提炼为通用 RT、view target 和 UI texture handle。
2. **Editor app skeleton / UI shell / viewport**：新增 editor host，先显示 RenderView 输出，不做 gizmo、inspector 或 asset browser。
...
| 13 | 通用 RenderTarget / TextureView | 从 `BasicOffscreenViewportTarget` 提炼通用 RT 描述：color/depth format、extent、usage、sampled texture view、resize/recreate 和 deferred deletion counters；命名保持 renderer/RHI 通用，不出现 editor 专属 API | offscreen viewport smoke 验证 RT 独立尺寸、多帧复用、resize deferred deletion、sampled target 输出；后续 depth/MSAA/MRT 不需要推翻 API |
| 14 | RenderView target recording | 引入 `RenderViewDesc` / `RenderViewTarget`，让 renderer 能写入传入的 imported target；旧 `recordFrame()` 保留为 swapchain view 的便捷包装 | 同一 renderer 路径可分别写 swapchain 和 offscreen target；graph pass/schema 复用，不复制 editor 专用渲染路径 |
| 15 | Sampled texture bridge | 新增通用 UI sampled texture bridge，注册 image view + layout + sampler 为 `UiTextureHandle`；它不关心来源是 Game View、Scene View、asset preview 还是 debug texture | smoke 验证 sampled target 可注册、更新、注销；renderer 不依赖 UI，UI 不拥有 scene RT 生命周期 |
| 16 | Editor app skeleton / UI shell | 新增 `apps/editor` 和 `packages/editor-core`；editor-core 只放 selection、transaction、panel registry 等非 RHI 逻辑；UI shell 提供 dock、menu、log、viewport placeholder | editor app 能启动空窗口；sample-viewer 不链接 editor package；runtime packages 不依赖 editor |
| 17 | Editor viewport host | editor 只作为 RenderView 和 UI texture 的消费者：panel size 驱动 target resize，渲染层输出 sampled target，UI bridge 显示 handle；dock chrome 不走 scene RenderGraph | editor viewport 能显示通用 RT 输出，resize 不触发 validation/lifetime 问题；sample-viewer offscreen smoke 与 editor viewport 复用同一套 API |
...
- 完整 editor 产品功能、完整 asset database、script VM、bindless、async compute 和 transient memory alias 必须保持在暂缓项，直到 RT / RenderView / UI texture bridge 这些前置小闭环已经稳定。

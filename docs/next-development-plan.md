...
| 4 | RenderGraph access/state 扩展 | `ShaderRead(fragment/compute)` 已接入，后续继续补 `DepthAttachmentRead/Write`、`DepthSampledRead`，并同步 Vulkan layout/stage/access 翻译 | `--smoke-rendergraph` 已验证 shader-read transition、shader stage/domain 和 Vulkan adapter 字段；后续 depth state 仍需新增 smoke，不引入实际 shader sampling |

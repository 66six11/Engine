...
| 12 | 性能诊断底座 | 在 P3 diagnostics v2 收尾后、P4 backend lifetime/cache 前插入；新增 profiler 数据模型、CPU scope、benchmark CLI、RenderGraph compile counters，并预留 GPU timestamp/debug label 接口 | `--bench-rendergraph` 可在 Release preset 下输出 warmup/frames/p50/p95/max 和 graph counters；`--smoke-*` 行为不变；editor 面板只保留技术约束，不进入当前里程碑 |

...
3. `architecture.md` - 模块边界、对象所有权、生命周期和 render graph 设计。
4. `rendergraph-mvp.md` - 首个 render graph 闭环的最小里程碑。
5. `coding-standard.md` - C++23、Vulkan、shader、同步和工程风格规范。
6. `package-architecture.md` - 类 Unity package 的模块化文件与包管理设计。
7. `encoding-policy.md` - UTF-8 BOM 使用边界、检查脚本和工具兼容性说明。
8. `project-management.md` - 仓库布局、任务拆分、门禁和风险表。

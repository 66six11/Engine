...
这里的“调用”指消费同一 typed contract 的正式产品实现。当前 Python verifier 只是仓库内开发验证/reference oracle，不随 Studio 或 Distribution 发布，也不得由 Editor、Launcher、Installer 或 Repair Executor 启动；产品接入必须由 C# 或 native 实现共享合同与诊断语义。
...
此外，可信 inventory 下的实际文件必须通过 Python product-payload 禁入政策。即使一个历史 generation 的 manifest、canonical
bytes、全部 size/SHA-256 与 `EngineGenerationId` 彼此自洽，只要库存包含 `.py`、`.pyc`、`.pyd`、`.whl`、Python
package/virtual-environment tree 或 interpreter/runtime artifact，状态仍为 `RepairRequired` 且不得产生
`VerifiedInstalledDistribution`。finding 使用 `distribution.repair.python-payload-forbidden`、稳定 owner/pointer 与
generation-relative 逻辑路径；验证保持只读，不删除、隔离或覆盖命中文件。
...
- `distribution.repair.python-payload-forbidden`：历史或外部 generation 含 repository-only Python product payload。
...
- 共享 Python product-payload fixture 的旧自洽 generation 全部进入 `RepairRequired`，合法 generation 零命中且前后 byte tree 不变；

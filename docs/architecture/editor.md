...
The V9 native stream still keeps at most one executing request, one latest pending replacement and one ready frame per
...
The Studio V9 request carries the view-local FOV axis, selected canonical UUID/`ViewStateRevision` and an optional
discriminated Translate/Rotate Gizmo packet. Selection and tool-mode changes participate in the hard content fence;

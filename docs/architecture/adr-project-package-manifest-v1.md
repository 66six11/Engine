...
回溯与 canonical exact graph 物化，[Explicit-source Package Candidate Discovery v1](adr-package-candidate-discovery-v1.md) 已实现
strict candidate loader，[Locked Graph Verification & Reuse v1](adr-package-lock-verification-v1.md) 已实现 existing exact graph 的
只读复用。上游 catalog/index、lock update/apply、生产 Feature Set/lockfile 或 Build/Activation Plan 仍未实现。

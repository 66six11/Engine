...
流式 staged verification 与不可变 artifact generation publication。上游 catalog/index、lock update/apply、Engine Distribution /
Effective Session contracts、Activation Plan、生产 catalog/lockfile 与 Editor Package Manager 尚未实现。
...
- Editor UI 不是 package graph 的事实来源；项目 `asharia.packages.json` / lock 与只读 Engine Distribution Manifest
  分别拥有项目依赖和发行库存，派生 Session/Composition 不成为第三份 lock。
...
- Package Manager 只生成 Host composition 及后继 build/activation plan，不拥有 World、Renderer、Script VM 或其他系统实例。
...
- 第一阶段的 native modules 默认保持独立 CMake 静态库/工具 target，由生成的薄 Host composition root 链接并在启动期
  注册；package、module、target 和 DLL 不是同义词。native graph 变化进入 `PendingBuild -> PendingRestart`。
...
```text
Engine Distribution Manifest + project asharia.packages.lock.json + Host Profile
...
[ADR：Package Product & Artifact Evidence v1](adr-package-product-artifact-evidence-v1.md)。

...
- `scene-core` 的 `asharia::scene_core` 提供 headless World、runtime EntityId 和 local Transform baseline；
  ...
  generation-safe entity create/destroy/is-alive。adapter 只私有依赖 runtime target，不公开 C++ STL/exception、
  entity name/Transform、managed bridge 或 render integration。

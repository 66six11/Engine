...
的合同/校验基线已经落地；deterministic in-memory Package Resolver v1 已能把 caller-supplied candidates 求解为 canonical exact
lock graph。candidate discovery、locked reuse/update、Build/Activation Plan、生产 catalog/lockfile 与 Editor Package Manager
尚未实现。
...
writer 与 synthetic fixtures 已实现。caller-supplied candidates 的选择策略、回溯、diagnostics 与 exact lock 物化见
[ADR：Deterministic in-memory Package Resolver v1](adr-package-resolver-v1.md)；candidate discovery、locked reuse/update、生产
Feature Set/lockfile 仍未实现。
...
[ADR：Deterministic in-memory Package Resolver v1](adr-package-resolver-v1.md)。

...
的 Product Declaration、candidate snapshot 与 pure per-package artifact verifier 已落地。上游 catalog/index、lock update/apply、
artifact collector/publication、Activation Plan、生产 catalog/lockfile 与 Editor Package Manager 尚未实现。
...
flowchart LR
    ...
    Composition["Per-host logical composition"]
...
Package Manager 的工作到 locked Host composition 与后继 build/activation plan 为止；它不负责执行项目产品构建。Editor 创建的项目由独立、headless 的 Project Build 与 Project Launch services 完成：

...
`asharia-static-factory-provider-v4` + RegistrationSnapshot v2。本 Slice 的 lifecycle state machine 未改变，也不保留旧 provider、
renderer、executor 或 schema adapter。[Static Contribution Payload Accessors v1](adr-static-contribution-payload-accessors-v1.md) 只让
callback table 保存 future payload projection authority；typed contribution payload/lease 与 accessor invocation 仍不属于 ProcessScope v1。

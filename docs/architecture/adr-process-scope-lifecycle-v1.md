...
当前 #296 已用 [ProcessScope Contribution Registry and Activation Lease v1](adr-process-scope-contribution-registry-and-activation-lease-v1.md)
后继并硬切本 ADR 的 public ProcessScope surface：executor、preparation/start/stop result、error、state、stage、diagnostic 与 exact factory
reference 统一使用 V2，不保留 V1 compatibility surface。本 ADR 冻结的五个 factory callbacks 及
`create-activate-quiesce-deactivate-destroy-v1` lifecycle model 仍保持 V1；本文后续出现的 `ProcessScopeExecutorV1` 等名称仅描述
#293 当时的 baseline，不是当前 public API。

#296 在同一 ProcessScope owner 中增加 fixed-slot typed registry 与 contribution-only lease。`activate` 成功后，Host 调用 selected
accessors 并原子提交该 factory 的 lease，随后 factory 才对 dependents 可见；全部 factories 成功后 registry 才开放。cleanup 顺序扩展为
reverse quiesce → registry `Revoking` → reverse lease revoke → reverse deactivate/destroy → registry `Revoked`。view/handle 只 weak-reference
该 generation，旧 handle 在 revocation、epoch 变化或 owner 销毁后 fail closed。
...
1. #296 已在 root ProcessScope 内实现 fixed-slot typed registry、weak generation view/handle、contribution-only lease、atomic publication、
   reverse revoke 与 stale-generation fail-closed；它没有把 lease 扩展到 jobs/subscriptions，也没有增加其他 scope owners。
2. 下一步必须连接一个真实 Host 与一个真实 system/contribution，形成可观察的 vertical feature；不再为尚无消费者的 Project/Editor/ToolJob
   scope 或通用 lease 能力继续扩展抽象 Foundation。
3. production Bootstrap/launcher issuer、normal Host adapter 与其他 concrete scope owner 仍需由真实功能需求驱动；它们不能把
   `ProcessScopeExecutorV2` 泛化为万能 scope。
4. 若 future native module 采用 exact-build DLL 或 threaded executor，单独冻结 module lease、unload gate、thread affinity、drain 与 callback
   ABI，不修改五 callbacks 与 lifecycle model v1 语义。

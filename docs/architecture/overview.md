...
- `apps/studio`：Avalonia managed Studio shell。当前产品链是 `App/Shell -> Application ProjectSession -> EngineBridge
  ...
  -> EngineBridge ViewportBridge V9 stream -> editor_native bounded scheduler -> process RenderThread -> renderer_basic_vulkan`
  ...
  已接入固定 2 px selection outline、mouse-only camera navigation 与单选世界轴 Translate/Rotate Gizmo；mesh triangle/GPU-ID picking、
  scale/local/snap/multi-select gizmo、WASD/focus-selected 仍未接入。
...
- `Asharia.Studio.EngineBridge.Viewports.ViewportBridge` 是 managed V9 stream ABI 边界；它复制 view-local FOV axis、最多 256 个
  ...
  不取得句柄；V1–V8 frame exports 与 managed fallback 均已删除。
...
- native `EditorSharedViewportRuntime` 是 process singleton，并拥有唯一 RenderThread、有界 V9 stream scheduler、
...
8. 每轮 frame 通过 `editor_viewport_complete_frame_v9(stream, slot, completionKind)` exact-once 完成；compositor submission 前拒绝

...
- `apps/studio`：Avalonia managed Studio shell。当前产品链是 `App/Shell -> Application ProjectSession -> EngineBridge
  ...
  已接入固定 2 px selection outline、mouse-only camera navigation、单选世界轴 Translate/Rotate 与局部轴 Scale Gizmo；mesh triangle/GPU-ID picking、
  plane/center-uniform/local translate-rotate/snap/multi-select gizmo、WASD/focus-selected 仍未接入。

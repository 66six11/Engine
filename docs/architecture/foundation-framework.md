...
下图描述 F2 的目标执行流。当前已实现的合同是 verified Engine Distribution + Project Lock v2，以及 Host Profile projection；
[Host Composition Plan v1](adr-host-composition-plan-v1.md) 从这两个输入生成 canonical logical IR，但不产生图中的可执行
`ordered activation plan`。Distribution/Project 所有权已分离；下一步以 Effective Session 将两者和 Host Profile
组合为 verified session input。Effective Session Plan 只派生状态和子计划 handoff，
不成为第三份 lock。
...
`engine-distribution` reference 绑定 exact generation。生产 Distribution assembly、Effective Session、factory contracts 与

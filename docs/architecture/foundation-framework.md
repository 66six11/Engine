...
下图描述 F2 的目标执行流。当前已实现的过渡合同仍是单一 verified lock 与 Host Profile projection；
[Host Composition Plan v1](adr-host-composition-plan-v1.md) 从这两个输入生成 canonical logical IR，但不产生图中的可执行
`ordered activation plan`。在设计该 plan 前，必须先引入只读 Engine Distribution Manifest，并把现有 `bundled` lock 节点
迁移为 Distribution 与 Project Lock 分属不同 owner 的组合输入。Effective Session Plan 只派生状态和子计划 handoff，
不成为第三份 lock。

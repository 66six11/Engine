...
并把 T3/C6 normal Host 接到这条 ProcessScope 路径；Editor Bootstrap state mapping 与其他 scopes 仍未实现。完整合同见
...
未来可以先用 ImGui 快速实现固定 Bootstrap UI，稳定后迁移到 Avalonia；这只是前端演进方向，不属于 #297 的 headless Host Slice。
...
下图描述 F2 planning 与 #297 已接通的 normal Host 执行流。verified Engine Distribution + Project Lock v2、Host Profile projection 与

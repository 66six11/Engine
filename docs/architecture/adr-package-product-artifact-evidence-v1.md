...
3. **Artifact verifier**：把 Effective Session 的 verified resolved graph、Host Composition、Source Build Plan、candidate declaration snapshots 与 caller-owned
...
- 不含空、`.` 或 `..` segment；
- 不使用 Windows reserved name/invalid character/control character 或尾随空格/点；
- 不占用 package root 的 `asharia.package.artifacts.json`；
- 在同一 package 内 exact unique，且不与其他 path 发生 Unicode case-fold 或 file/directory ancestor collision。

verifier 从 observation 的 exact `bytes` 重新计算 size 与 SHA-256，并与 caller claim 对证。v1 的 byte observation 是纯 control-plane
测试/适配合同；[#278 collector/publication ADR](adr-package-artifact-collection-publication-v1.md) 已冻结 owned streaming
boundary：collector 分块复制、独立重哈希 staged bytes 后才通过私有 descriptor handoff 复用本 verifier 的其余逻辑。
公共 verifier 仍不相信 caller 提供的未对证 size/hash 字符串。

### 3.3 产品 artifact 不接受 Python payload

Package Artifact Manifest 的 portable path 合法性不等于产品资格。Python 只用于仓库内开发、验证与 CI；正式 package
artifact 不得声明 `.py`、`.pyc`、`.pyd`、`.whl`，`site-packages`/virtual-environment tree，`python*.exe`、
`python*.dll`/import library 或 `libpython*`/PyPy runtime artifact。该判断只基于已规范化的 package-relative logical path，大小写不敏感，
不做内容嗅探，也不把 Python runtime 加入产品依赖。

Pure verifier 在生成 manifest 前对全部 file evidence 原子检查。任一命中返回稳定的
`artifact.python-payload-forbidden` diagnostic，并保持 `manifests=()`；同一语义也用于后继 publication receipt 的深度
复验。因此历史 manifest 即使 schema、self-integrity、canonical bytes 与 manifest-set identity 全部自洽，也不再具有
产品资格。该政策不改变 closed schema、canonical serialization 或 content-ID 算法。
...
- `VerifiedResolvedGraph` normalized 且 fingerprints 有效，Source Build Plan package/version/module selection 与 Host Composition 相同；
...
Package Artifact Manifest 也不是 Editor Bootstrap 或日常源码编辑的唯一状态依据。固定 Editor Image 先于项目图启动；
Engine Distribution Manifest、Project Lock 与 Host Profile 派生 Effective Session Plan。`project-embedded` / `local` package
开发时以 source root 为当前真相，只有 build/install/cache/repair/activation 边界才生成并验证 immutable artifact generation。
完整所有权见
...
- 共享 Python product-payload fixture 的全量拒绝、合法 .NET/native/package controls 零命中，以及旧自洽 manifest fail closed；
...
- Discovery → Resolve → Effective Session → Host Composition → Source Build Plan → Artifact Manifest synthetic handoff。

## 后续边界

1. **#278 Explicit artifact collector / atomic publication**：从明确、quiescent install/acquisition root 流式收集 bytes
   evidence，发布 package artifact generation；它不拥有 Editor Bootstrap 或 Session composition；
2. **Engine Distribution / Effective Session contracts**：`EngineGenerationId`、只读发行库存、Project Lock v2 与
   Ready/Upgrade/Repair/SafeMode session 已完成；Safe Mode UI/进程与轻量启动检查后置；
3. **Factory / Scope / Lifecycle contract**：冻结可执行 entry/factory、scope、phase、service dependency、lease 与 rollback；
4. **Activation Plan**：对证 Distribution、Project Host Composition、verified package artifacts 与 factory contracts；
5. **Project Stage v1**：由 Project Product Pipeline 生成最终 runnable `asharia.stage.json`；
6. **Acquired/prebuilt evidence**：在 trust/signature/source policy 冻结后，让 artifact-only candidate 复用同一 Package Artifact Manifest shape。

以上必须保持独立 Slice。#277 本身不实现 collector、Distribution、Activation 或 stage；#278 只承接其中第一项。

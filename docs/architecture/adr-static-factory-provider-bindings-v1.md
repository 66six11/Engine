...
[Static Contribution Payload Accessors v1](adr-static-contribution-payload-accessors-v1.md) 又由 #295 完成后继硬切：author bindings 与
Binding Plan 为 v4、provider API 为 v4，selected `StaticContributionBindingV2` 同时携带 typed payload accessor；v3 只保留为 #294
历史合同，仍不提供兼容 reader 或 adapter。
...
schema/model v2，且不保留 v1 reader、schema、adapter 或 executable path。本 ADR 仅记录 #286/#291 的历史决策；
#294 的 v3 合同同样已成为历史，当前 #295 v4 合同由本文开头链接的后继 ADR 定义。
...
- 不生成构建后 Host Executable Binding Receipt；
...
3. [Windows Development Host Template v1](adr-windows-development-host-template-v1.md)：#290 已实现固定 final target、`main()`、
   受控构建、File API target binding 与 registration-only verification；
4. [Host Executable Binding Receipt v1](adr-host-executable-binding-receipt-v1.md)：#288 已对证 Blueprint、registration snapshot 与
   exact staged Host artifact generation；
5. [Static Factory Callback Table v1](adr-static-factory-callback-table-v1.md)：#291 绑定 current-process typed callbacks，并保持
   identity snapshot/receipt 不含 pointer；
6. Activation Eligibility 与 Host Runtime lifecycle：先授权 long-lived Host 调用 providers，table 对证后再授权 callback access，
   随后实现 scope tree、factory context、activation lease、rollback 与 shutdown；
7. Bootstrap adapter：把 build/binding/runtime 结果映射为 Ready、PendingBuild、PendingRestart 或 SafeMode。

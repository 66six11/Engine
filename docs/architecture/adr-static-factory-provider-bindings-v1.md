...
### 2. 固定 provider API 版本，不提前实现 Host Runtime
...
`StaticFactoryRegistrar` 的 callback、context、instance 与 lifecycle API 不属于本 Slice；它们由未来 Host Runtime contract 冻结。provider entry point 只注册 factory 实现，不创建、激活或销毁 system instance。
...
- 不定义 `StaticFactoryRegistrar` 的具体 callback/registry API；
...
1. generated static composition root：消费 verified `StaticFactoryProviderBindingPlan` 并复验其 Source Build Plan/Blueprint fingerprints，生成薄 C++ registration source、compile-time provider signature checks 和 CMake target glue；
2. post-build Activation Binding Receipt：对证 Blueprint、实际 registration table 与 exact host artifact generation；
3. Host Runtime：实现 registrar、typed callbacks、scope tree、factory context、activation lease、rollback 与 shutdown；
4. Bootstrap adapter：把 build/binding/runtime 结果映射为 Ready、PendingBuild、PendingRestart 或 SafeMode。

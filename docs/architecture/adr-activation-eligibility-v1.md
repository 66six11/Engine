...
当前唯一接受的生成组合仍是 Windows Development Host Template renderer 2 + Static Composition renderer 3 +
`asharia-static-factory-provider-v2`。这不是旧版兼容政策；其他 tuple 一律拒绝。Receipt v1 与
RegistrationSnapshot v1 仍是当前 evidence schema，也不表示保留旧 generated Host reader 或 adapter。
...
| generation tuple | exact Template 2 / Composition 3 / provider v2；没有 fallback |
...
为了不修改 current renderer 2/3 generated bytes，normal long-lived path 使用 Host Runtime wrapper 接入现有 generated function：
...
#292 采用 wrapper 而不修改 generated composition/header/template bytes，所以 current tuple 保持 T2/C3/provider-v2。若实现发现必须修改

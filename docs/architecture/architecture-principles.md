...
作为 typed params 与 command summary 记录；存在 `BasicDebugWorldLine` 时，它会在 `renderer_basic_vulkan`
内投影 world line 并通过 debug-line shader/pipeline 绘制 line-list。现有 mesh/draw-list smoke 仍在 renderer
内部用 extent 和 model matrix 计算 MVP。后续相机相关 grid、scene mesh、selection、gizmo 或 debug line pass
必须继续沿 renderer-owned per-view constants / pass input 合同推进，不能回退到 diagnostics-only 数据源。

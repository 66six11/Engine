...
桥接到 `BasicRenderViewCamera`；`BasicFullscreenTextureRenderer::recordViewFrame()` 现在会在 overlay enabled
时插入 `builtin.render-view-overlay` pass，把 camera position / near plane、frame params、debug world-line count
作为 typed params 与 command summary 记录，并通过 dynamic-rendering load/store 触碰 view target。它仍不是可见
grid/debug-line renderer；现有 mesh/draw-list smoke 仍在 renderer 内部用 extent 和 model matrix 计算 MVP。后续
相机相关 grid、scene mesh、selection、gizmo 或 debug line pass 必须继续沿 renderer-owned per-view constants /
pass input 合同推进，不能回退到 diagnostics-only 数据源。
...
`BasicRenderViewOverlayDesc`。这不是 runtime camera system、完整相机交互、camera-aware grid、renderer
per-view constants，也不是可见 GPU grid pass。

下一步顺序：

1. 让 provider 参数来自 manifest/settings-ready 数据。
2. 补最小 Scene View orbit/pan/dolly 输入，并让相机变化触发 `CameraInputChanged` repaint reason。
3. 增加 renderer-owned per-view constants / pass input 合同，再实现 camera-aware range/fade policy。
4. 增加 renderer-owned debug line graph pass。
5. 让 Frame Debug / RG View 显示 pass、packet count、view kind 和 selected event。

...
桥接到 `BasicRenderViewCamera`；但当前 `BasicFullscreenTextureRenderer::recordViewFrame()` 只把 camera 回显到
diagnostics，现有 mesh/draw-list smoke 仍在 renderer 内部用 extent 和 model matrix 计算 MVP。后续相机相关 grid、
scene mesh、selection、gizmo 或 debug line pass 必须先补 renderer-owned per-view constants / pass input 合同。

...
本文是 [Host Composition Plan v1](adr-host-composition-plan-v1.md) 和
[Host Activation Blueprint v1](adr-host-activation-blueprint-v1.md) 之间的作者输入边界。
...
跨 package factory 是否存在、跨 package cycle 与最终 Host selection 完整性，由 Host Activation Blueprint 在 exact selected graph 上验证；
...
Host Activation Blueprint 对 ready factories 使用 dependency-first topological order；多个同时 ready 的 factory 再按 canonical
...
它只保存经过验证的后继输入，不生成 Host Activation Blueprint。

sidecar 在 Discovery 层保持 optional，是为了允许 Feature Set、数据包和迁移期 package 继续被解析。Host Activation Blueprint
对每个 Host-selected installable package 强制要求 declaration；没有 factory 的 module 必须显式写 `no-factories`。该条件不提前塞进 resolver。

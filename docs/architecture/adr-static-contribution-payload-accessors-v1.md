...
process-local C++ type key，但尚无办法从一个成功激活的 factory instance 取得该 contract 的 payload。#295 只补齐这条
registration-time runtime binding，不调用 accessor、不发布 payload，也不实现 registry 或 `ActivationLease`。

[ProcessScope Lifecycle v1](adr-process-scope-lifecycle-v1.md) 的五个 lifecycle callbacks、instance token ownership、startup
rollback 与 explicit stop 语义保持不变。下一 Slice 才会在 admitted `ProcessScope` 中于 `activate` 成功后调用 selected
accessors，完成 staging 与 atomic commit。
...
下一 Slice 若某个 selected accessor 返回 null，Host 必须把它视为 activation publication failure：该 factory 尚不得成为
dependency-visible，也不得留下 committed contribution；但因为其 `activate` callback 已经返回 success，rollback 仍必须对该 factory
执行适当的 quiesce/deactivate/destroy cleanup。这一执行状态不在 #295 实现。
...
下一 Slice 在 admitted ProcessScope 中消费 callback-table private runtime bindings：
...
5. rollback/stop 在 instance destroy 前按 reverse owner order revoke leases，旧 scope-generation handle fail closed；
6. accessor-null/commit failure 发生在 `activate` success 之后时，failed factory 仍完成 quiesce/deactivate/destroy，但不成为
   dependency-visible；
7. 不向 factory context 暴露 arbitrary `getService<T>()`，不建立 process-global registry。

其他 concrete scopes、production Bootstrap/Session adapter 与动态 contribution mutation 继续作为独立后续工作。

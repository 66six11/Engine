...
- `scene-core`、`asset-core` 和 `material-core` 目前是 CPU/headless 数据模型 package，不依赖 renderer、RHI 或 editor；
  ...
  version/struct-size 校验后的 opaque World 生命周期与 generation-safe entity create/destroy/is-alive，并强制
  所有操作由创建线程调用。它尚不公开 entity name/Transform、managed binding 或 render extraction。

...
- `scene-core`、`asset-core` 和 `material-core` 目前是 CPU/headless 数据模型 package，不依赖 renderer、RHI 或 editor；
  ...
  version/struct-size 校验后的 opaque World、generation-safe entity 生命周期与 validated local Transform
  get/set，并强制所有操作由创建线程调用。它尚不公开 entity name、hierarchy/world Transform、managed
  binding 或 render extraction。

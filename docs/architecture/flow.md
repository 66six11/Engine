...
- `apps/studio` 是 Avalonia managed Studio shell，不属于 C++ CMake target graph。Project/document 产品链为
  ...
  产品链为 `StudioScenePanelView -> ViewportCompositionControl -> Application ViewportSession -> EngineBridge V10 stream
  ...
  V10 canonical UUID -> selected draw packet -> Selection Mask -> Outline Composite`。Avalonia content gate 同时核对 target、view-state
  ...
  existing V10 camera packet`。Application 数学不引用 Avalonia，gesture 不写 selection/document；native ABI、renderer 与 RenderGraph
  无新增相机导航分支。
  Scene View 变换走第三条输入链：`presented identity + selected ObjectId -> Application axis/ring hit + drag -> transient
  ViewportSession GizmoChanged -> V10 discriminated Transform Gizmo packet -> renderer-owned debug world lines -> pointer release -> one
  ...
  V1–V9 frame exports 已硬切删除；Vulkan context、producer、queue submit、retirement 与 shutdown 只由 native owner thread
...
#411 再以 V9 增加 discriminated Transform Gizmo packet，#413 以 V10 给 packet 增加 normalized rotation 并接入 local-axis Scale；
...
Scene schema v2 / Document ABI v3 snapshot
  ...
  -> V10 owning frame packet(source revision + FOV axis + optional Transform Gizmo)
...
revision context；空输入产生零 draw。相反，malformed V10 packet 是 ABI 边界失败，native scheduler 拒绝整帧而不提交部分内容。
...
sequenceDiagram
    ...
    participant Native as editor_native V10 stream ABI
    ...
    opt completion kind is known
        Bridge->>Native: editor_viewport_complete_frame_v10(stream, slot, completionKind)
...
- native V10 request/ready frame 是 self-described、owning ABI packet，并携 view-local FOV axis、selected canonical UUID/
  presence、`ViewStateRevision` 与 optional discriminated Transform Gizmo；V1–V9 frame
  exports 与 managed fallback 均已删除。V10 smoke 覆盖 burst request 只留下最新 sequence、ready 被占用时不覆盖、
...
- V10 completion hard cut 只有 `editor_viewport_complete_frame_v10(stream, slot, completionKind)`。未进入
...
- `editor_viewport_query_runtime_stats_v2..v7` 与当前 `v10` 是仅供 native smoke / diagnostics 的版本链，不属于 V10 stream
  compatibility surface；当前 smoke 使用 v10 记录 epoch、renderer creation/reuse、backpressure、已消费 scene frame、最后
  scene revision、Scene/Game/Preview frame count，以及最后一次 session/target/revision/sequence/render-kind/debug-proxy count、
  Scene world-grid 开关与实际 debug world-line count。V1–V9 stream exports 不再导出，也没有 managed fallback。
...
- `--smoke-render-view-scene-mesh` 已接入真实 RenderView scene-mesh 路径：deterministic validation
  ...
  Solid/Wireframe 与 FOV axis 是独立 per-view policy；Wireframe capability 缺失在 V10 submit 复制/入队前返回 typed

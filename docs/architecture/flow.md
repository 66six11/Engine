...
- `apps/studio` 是 Avalonia managed Studio shell，不属于 C++ CMake target graph。Project/document 产品链为
  ...
  产品链为 `StudioScenePanelView -> ViewportCompositionControl -> Application ViewportSession -> EngineBridge V8 stream
  ...
  V8 canonical UUID -> selected draw packet -> Selection Mask -> Outline Composite`。Avalonia content gate 同时核对 target、view-state
  ...
  existing V8 camera packet`。Application 数学不引用 Avalonia，gesture 不写 selection/document；native ABI、renderer 与 RenderGraph
  无新增相机导航分支。
  Scene View 世界轴平移走第三条输入链：`presented identity + selected ObjectId -> Application axis hit/drag -> transient
  ViewportSession GizmoChanged -> V8 typed Gizmo packet -> renderer-owned debug world lines -> pointer release -> one
  ProjectSession transform edit`。move sample 不写 document、不推进 hard presentation fence；Escape/capture/focus/stale revision 取消，
  mutation failure 回滚 authoritative Transform；没有 Physics、Avalonia overlay 或通用 Update/timer。
  V1–V7 frame exports 已硬切删除；Vulkan context、producer、queue submit、retirement 与 shutdown 只由 native owner thread
  ...
  rotate/scale/local/snap/multi-select gizmo 或 GPU ID-buffer/triangle geometry picking。当前 input consumer 已覆盖实际呈现 validation model 的 bounds picking、无模型实体的
...
Scene schema v2 / Document ABI v3 snapshot
  ...
  -> V8 owning frame packet(source revision + FOV axis + optional Translate Gizmo)
...
revision context；空输入产生零 draw。相反，malformed V8 packet 是 ABI 边界失败，native scheduler 拒绝整帧而不提交部分内容。
...
sequenceDiagram
    ...
    participant Native as editor_native V8 stream ABI
    ...
    opt completion kind is known
        Bridge->>Native: editor_viewport_complete_frame_v8(stream, slot, completionKind)
...
- native V8 request/ready frame 是 self-described、owning ABI packet，并携 view-local FOV axis、selected canonical UUID/
  presence、`ViewStateRevision` 与 optional typed Translate Gizmo；V1–V7 frame
  exports 与 managed fallback 均已删除。V8 smoke 覆盖 burst request 只留下最新 sequence、ready 被占用时不覆盖、
...
- V8 completion hard cut 只有 `editor_viewport_complete_frame_v8(stream, slot, completionKind)`。未进入
...
- `editor_viewport_query_runtime_stats_v2..v8` 是仅供 native smoke / diagnostics 的历史版本链，不属于 V8 stream
  ...
  Scene world-grid 开关与实际 debug world-line count。V1–V7 stream exports 不再导出，也没有 managed fallback。
...
- `--smoke-render-view-scene-mesh` 已接入真实 RenderView scene-mesh 路径：deterministic validation
  ...
  Solid/Wireframe 与 FOV axis 是独立 per-view policy；Wireframe capability 缺失在 V8 submit 复制/入队前返回 typed

...
13. RenderView 现在携带 renderer-owned view kind、camera/view/projection params、per-view frame params、overlay
    color load/store、blend mode 和 data-only debug world-line route。Scene View request 现在携带 editor-only
    camera context，并在 `EditorViewportCoordinator` 边界 bridge 到 `BasicRenderViewCamera`；renderer/basic 不消费
    `EditorViewportOverlayFlags`、ImGui state 或 editor camera state。`EditorViewportOverlayProvider` v0 可以收到
    camera context，但当前仍只生成固定原点 XZ grid packet。camera controls、viewport unproject、camera-aware grid
    policy 和可见 debug line pass 后续再通过这条 route 接入。

...
完整复验与不可变 generation publication；
[Installed Distribution Repair Verifier v1](adr-installed-distribution-repair-verifier-v1.md) 已由 #283 实现 trusted expected ID、
installed-tree 深度复验与只读健康报告。当前尚未实现 installer/launcher repair executor、轻量启动 receipt、
...
Manifest 与 hashes 是 immutable generation 激活的证据，不是 source file watcher。assembler 与 installed verifier 已在以下
边界完整验证：
...
本 Slice只定义 inventory contract 与 pure validation；#282 已实现 assembly filesystem boundary，#283 已实现显式
installed Verify/Repair 深度复验。轻量 startup receipt 仍未实现。
...
2. Distribution Assembler v1 与 Installed Distribution Repair Verifier v1 已完成；
3. 独立设计轻量启动 receipt / Bootstrap adapter，或生成静态薄 composition root；

...
[Project Manifest 与 Package Lock v2](adr-project-manifest-lock-v2-hard-cut.md) 单独设计，不能用一个空的 installable capability
伪装成完整能力。
...
合同。[Package Product & Artifact Evidence v1](adr-package-product-artifact-evidence-v1.md) 已为 #277 冻结独立
`asharia.package.products.json` 与 generated per-package Artifact Manifest，并用纯 verifier 对证 exact bytes。该分层允许后续在不修改
package identity 的前提下支持 CMake、预编译 native artifact、managed assembly 与 content-only delivery。
...
logical module 到 source build roots 的映射属于 build descriptor/Source Build Plan；logical product intent 与已验证 files 的映射属于
Product Declaration/Package Artifact Manifest。lock v1 不复制这些 platform/configuration-specific facts。
...
1. source-boundary v1 与 package author contract v2 可以并存；topology validator 只审计 source-boundary v1，package
   contract dispatcher 审计 installable v2、Feature Set v2 与独立的 Project Manifest v2。

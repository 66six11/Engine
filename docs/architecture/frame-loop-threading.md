...
production 已硬切到 V8 stream ABI。caller 把借用字段复制为 owning `RenderFramePacket`，`submit_latest_v8` 原位覆盖
每 stream 唯一 pending request 并立即返回；RenderThread 在 ready cell 为空且 slot 可推进时消费。`try_take_ready_v8`
同样不等待 GPU，ready frame 通过 producer-finished semaphore 把完成依赖交给 compositor。旧 V1–V7 frame exports
...
V8 packet 带 view-local FOV axis、immutable scene `sourceRevision`、backend-neutral `scene-rendering` 抽取结果与 optional typed
Translate Gizmo；Gizmo 只含 stable object identity、world position、hover/active axis，并复用 renderer debug world-line route。它不携带 scene/runtime
指针、asset importer state、resource registry reference 或 GPU key。Document ABI v3 与 V8 是硬切边界：旧 v1/v2
...
control/release mailbox 与 V8 stream scheduler 都有固定上限。互斥只保护队列、stream 状态与 diagnostic snapshot，
...
signal。stream close 时，managed 先 dispose persistent imports 并发送 `release_slot_import_v8`；native 再以 producer fence

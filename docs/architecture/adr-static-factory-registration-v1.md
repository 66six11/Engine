...
Host Template、test harness 或未来 verification mode 只负责调用这两个 generated entry points；它们不得重新枚举或构造
provider list。
...
#290 的 Host Template/Build Adapter 将提供 final executable target 与受限 verification entry；#288 的 post-build receipt 才把
composition manifest、registration snapshot 与 exact Host artifact bytes 交叉绑定。
...
1. #290 Host Template/Build Adapter：拥有 final target、`main()`、platform metadata、verification mode 与 final configure/build；

...
当前 v1 从 verified Project Lock v2 与 Host Profile 生成 IR；Project Lock 已精确绑定 Engine generation，但本 planner
尚未直接拥有 verified Distribution/Profile 的会话级组合证据。下一 Slice 将按
[Editor Image、Engine Distribution 与原生组合 ADR](adr-editor-engine-distribution-and-native-composition.md) 引入 Effective Session，
并让本计划消费其 verified resolved graph。
...
| [O3DE Gem Module System](https://docs.o3de.org/docs/user-guide/programming/gems/overview/) | 实际 module load 需要已知 entry points、create/destroy 与 initialize/uninitialize lifecycle | Activation Plan 必须绑定 factory/artifact/lifecycle，不能只携带逻辑 ID |
...
3. **Activation Plan**：消费 composition + verified artifacts + factory/scope/lifecycle contracts，供 Host Runtime 执行。
...
第一版 Python 实现将使用 snake_case 命名，但合同不依赖实现语言。`verifiedGraph` 是
`LockedGraphVerificationResult` 的成功值，必须同时满足：

- `succeeded == true`；
- `lock` 非空且已经 normalized；
- `diagnostics` 为空；
- `selectedCandidates` 对 lock 中每个 node 恰好提供一个 exact ID/version/kind/source-bound payload；
- 每个 candidate 的 author manifest 已通过 lock cross-document 与 payload integrity verification。

planner 不接受彼此独立的 raw lock/candidates 参数，避免调用方绕过 #274 的 exact source 与 integrity proof。planner 仍应重新检查输入
result 的结构不变量与 exact candidate/manifest binding，防止人工构造的 incomplete result 被误用。`project` 必须通过 Project
Manifest v1 validation，其 normalized digest 必须等于 lock 的 `inputs.projectManifestIntegrity`；planner 不重新求解版本、不枚举来源、
不重新读取 payload，也不写文件。

如果上游 verification 已失败，planner 原样保留稳定的上游 diagnostics 并返回空 plan。unsupported/forged/incomplete result 使用
`plan.input.unverified` 或 `plan.input.incomplete`，不会触发 resolver fallback。

```mermaid
flowchart LR
    Verified["Verified exact graph\nlock + selected candidates"]
...
当前 planner 从 verified Lock v2 读取 `engine-distribution` nodes，但不会从 lock 读取发行 path/hash；这些 evidence 只属于
Engine Distribution，并在 locked verification 阶段完成绑定。后继 Effective Session composer 必须分别验证 Distribution、
Project Lock 与 Host Profile，再把 verified resolved graph 提供给 Host Composition。Host Composition Plan 不复制 Distribution，
也不升级为第三个 lock。
...
#274 证明的是 verification 时刻的 mutable source roots。Host Composition Plan 只读取该 verified in-memory snapshot，不读取 binary、
source tree 或构建产物；因此本 Slice 不重复 payload hashing。
...
2. 新增只接受 successful `LockedGraphVerificationResult`、matching Project Manifest 与 Host Profile 的 pure planner；
...
5. 增加 Discovery -> Resolve/Locked Reuse -> Host Composition Plan 的 synthetic handoff；
...
2. Factory / Scope / Lifecycle contract：可执行 Activation Plan 的必要前置；

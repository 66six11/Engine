...
Package Lockfile v1 有意不复制 package options；effective values 必须由 pinned installable manifest defaults 与 normalized Project
...
当前 v1 planner 仍从一份 verified lock 读取 `bundled` nodes，这是迁移期兼容事实，不表示 lock 最终拥有 Engine/Editor
发行库存。[Engine Distribution Manifest v1](adr-engine-distribution-manifest-v1.md) 已将固定 inventory 与
`EngineGenerationId` 分离；后继 Effective Session composer 必须先分别验证 Distribution、Project Lock 与 Host Profile，再把
兼容的 bundled identities 提供给 Host Composition。Host Composition Plan 不复制 Distribution，也不升级为第三个 lock。

...
- Scene View request 已携带 editor-only camera context；provider context 可以读取 camera/view/projection 数据。
...
- camera controls / viewport unproject 语义落地后，Grid provider 再生成 camera-aware XZ grid line packets。

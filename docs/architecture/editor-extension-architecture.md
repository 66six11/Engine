...
- 这一步证明 provider contract、editor-to-RenderView 数据桥、camera/unproject 基础、diagnostics count 和
  renderer-owned debug-line GPU path。它还不是 manifest-backed provider；grid spacing/color/settings 仍待
  settings 化，但 camera-aware grid spacing、无默认距离淡出、高视角 readback 和 source overlay id diagnostics
  已有最小闭环。
...
- Done: source overlay id 进入 diagnostics，方便多个 provider/debug packet 溯源；renderer 只复制 caller-owned
  id，不把 editor id 解释为 graph/pass 语义。

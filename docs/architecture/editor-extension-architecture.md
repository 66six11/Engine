...
- 这一步证明 provider contract、editor-to-RenderView 数据桥、camera/unproject 基础、diagnostics count 和
  ...
  opacity 和 color 已从 editor user settings 进入 RenderView，Scene grid overlay contribution 已声明 built-in 默认值，
  且 built-in tool contribution 已先经过 `EditorExtensionRegistry` v0 再发布给 `EditorToolRegistry`。外部
  manifest loader、enable/disable、reload diagnostics UI 和热更新仍待脚本/插件系统边界明确后再补；camera-aware
  grid spacing、无默认距离淡出、高视角 readback、settings UI v0 和 world-grid/source overlay diagnostics 已有最小闭环。
...
- 默认 Scene View grid 已改为 renderer-owned fullscreen world-grid pass，不再回退到 camera-aware XZ grid line
  packets。
- Grid spacing、major spacing、fade、opacity 和 color 来自 editor user settings，并通过 Scene View request 进入

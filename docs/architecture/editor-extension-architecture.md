...
- 这一步只证明 provider contract、editor-to-RenderView 数据桥、camera/unproject 基础和 diagnostics count；
  当前还没有 renderer-owned per-view constants / pass input 合同。它还不是 manifest-backed provider、
  完整 Scene View 相机交互、camera-aware grid policy，也没有 renderer-owned debug line graph pass 或可见
  GPU 线框绘制。

...
- 这一步证明 provider contract、editor-to-RenderView 数据桥、camera/unproject 基础、diagnostics count 和
  ...
  grid color、外部 manifest loader、设置 UI 和热更新仍待补齐；camera-aware grid spacing、无默认距离淡出、高视角
  readback 和 world-grid/source overlay diagnostics 已有最小闭环。
...
- Grid spacing、major spacing、fade 和 opacity 来自 editor user settings，并通过 Scene View request 进入
  ...
  使用该默认值处理新文件和旧文件迁移；external manifest loader、color 和热更新仍是后续工作。

...
- provider 当前输出固定原点 `scene.grid` packet，renderer 已通过 RenderView overlay debug-line path 消费并绘制。
...
- 当前固定 grid packet 已收敛进 provider contract 并有可见 renderer pass；下一步是 camera-aware range/fade
  policy，而不是继续把 Vulkan 绘制细节推回 editor。

...
- `apps/editor/src/editor_viewport_coordinator.hpp:150` 使用 `std::optional<EditorViewportRequest> requestedViewport_`。
- `apps/editor/src/editor_viewport_coordinator.cpp:184` 每帧 reset。
- `apps/editor/src/editor_viewport_coordinator.cpp:189` 到 `:197` 的 `requestViewport()` 会覆盖上一条 request。
- `apps/editor/src/editor_viewport_coordinator.cpp:217` 到 `:235` 的 `recordRequestedViews()` 只消费这一条 request。

风险：

当前单 Scene View 可以工作，但同帧 Scene/Game/Preview、多 panel viewport、asset preview 和 multi-view diagnostics 都会被“最后一次请求 wins”破坏。这个问题属于内部状态模型，不是 package 边界问题。
...
2. 再修 P2-B / P2-C：per-view constants / debug-line pass 与 keyed multi-view request model。它们决定 editor viewport 是否能支撑 Scene/Game/Preview。

...
binding 与 registration-only verification。它尚不构成 stage、launch session、artifact receipt 或 runtime lifecycle。
...
identity snapshot。table 不公开未授权的 raw callback lookup；#292 已实现 headless Activation Eligibility C++ boundary：对证
receipt、session 与 current process 后才允许 long-lived Host 调用 providers，table snapshot 再次对证后才允许 lifecycle executor
访问。production launch issuer 与 normal Host 接线仍未实现。#288 的
...
6. 只有 Effective Session `Ready` 才可进入 [Activation Eligibility v1](adr-activation-eligibility-v1.md)；后者必须先取得一次性
   `PreRegistrationAdmissionV1`，再将同一次 recording 的 table 对证为 `ActivationAdmissionV1`。项目 modules/contributions 只有在
   future #293 lifecycle 成功后才算激活；当前 production launch issuer 与 executor 均未实现；
...
- Engine Distribution Manifest v1、`EngineGenerationId`、Project Manifest / Lock v2 硬切、Effective Session v1、
  ...
  Template renderer 2 + Composition renderer 3/provider v2；#292 已添加 Activation Eligibility validator、linear wrappers 与 focused
  tests，最终门禁结果以 Done evidence 为准；production launch issuer 仍待实现，#293 再实现 admitted
...
2. generated composition root 的 target/module registration 与两阶段 CMake handoff 已由 #287 实现，#289 的 single-argument /
   ...
   target/compiler 与 owning snapshot 交叉绑定；Host build、assembly 与 deep verifier 只接受当前 T2/C3/provider-v2 组合；#292
   Activation Eligibility C++ boundary 已实现但仍需完成门禁记录，后续仍需 production launch issuer、normal Host 接线、#293
   ProcessScope lifecycle executor 与 Bootstrap/Session adapter；

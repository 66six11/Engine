...
- renderer revision 5 生成 exact registration capacity、composition/provider context、expected local factory/contribution ID/kind、
...
- Engine Distribution Manifest v1、`EngineGenerationId`、Project Manifest / Lock v2 硬切、Effective Session v1、
  ...
  API 硬切为 v4，不保留 pre-current reader/adapter；#287 的 generated root 当前只生成 renderer 5/provider v4 的 content-addressed thin TU 与受控
...
2. generated composition root 的 target/module registration 与两阶段 CMake handoff 已由 #287 实现，#289 的 single-argument /
   identity-only registrar contract 是历史表面；#295 当前只接受 Composition renderer 5/provider v4，把 exact factory identity、完整
   ...
   Snapshot v2。固定 Windows Development
   ...
   target/compiler 与 owning snapshot 交叉绑定；Host build、assembly 与 deep verifier 只接受当前 T2/C5/provider-v4/Snapshot-v2
   组合；#292

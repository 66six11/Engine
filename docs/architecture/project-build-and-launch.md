...
- [Windows Development Host Template v1](adr-windows-development-host-template-v1.md) 已为 #290 提供固定 final executable、
  ...
  Host target、不使用 clean-first，构建完成后仍由 #288 receipt 把 composition generation、registration snapshot 与 exact
  Host artifact bytes 绑定；
...
- Engine Distribution Manifest v1、`EngineGenerationId`、Project Manifest / Lock v2 硬切、Effective Session v1、
  ...
  File API target binding 与 registration-only verification；后续仍需 #288 activation binding receipt 与 Bootstrap/Session adapter；
...
2. generated composition root 的 target/module registration 与两阶段 CMake handoff 已由 #287 实现，identity-only recording
   ...
   verification 已由 #290 实现；仍需 #288 构建后 activation binding receipt，以及 verifier 到 Bootstrap/Session 的 adapter；

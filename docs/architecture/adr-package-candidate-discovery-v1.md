...
[Project / Local Package Source Catalog v1](adr-project-local-package-source-catalog-v1.md) 已由后继 #302 实现 Project/local
显式位置 provider；它复用本文 loader，不把 source index、机器映射或 precedence 下沉到 loader。
...
package index 和 local workspace mapping 是它的上游位置提供者，不属于 #273；bundled provider 已由 #301 实现，Project/local
provider 已由 #302 实现。
...
```mermaid
flowchart LR
    Providers["Source providers\nverified Distribution catalog #301\nProject/local source catalog #302"]
    ...
    Locked["Locked verify/reuse #274 implemented\nupdate/apply future"]
...
- bundled distribution catalog、project package index 或 workspace mapping 文件格式（分别由后继 #301 / #302 定义）；
...
重哈希；[Engine Distribution Package Catalog Snapshot v1](adr-engine-distribution-package-catalog-snapshot-v1.md) 已为 #301 实现
verified Distribution bundled provider；[Project / Local Package Source Catalog v1](adr-project-local-package-source-catalog-v1.md)
已为 #302 实现 portable Project source index + process-local mapping provider。Lock update/apply 与 production Package Manager 仍未实现，
不能由本 Slice 的 strict loader 或两个 source providers 推断为已落地。
...
- GitHub #264、#270、#271、#272、#273 与 #302

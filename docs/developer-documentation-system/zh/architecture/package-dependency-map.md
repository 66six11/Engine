...
- `packages/shader-material-adapter` 的 test target 可以使用 `shader-authoring` fixtures，但 adapter library target 只链接 `asharia::core`、`asharia::material_core` 和 `asharia::shader_slang`。

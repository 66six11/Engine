...
namespace {
    ...
    void drawSummary(const asharia::editor::EditorFrameDebugger& frameDebugger,
                     const asharia::editor::EditorFrameDebugCapture& capture,
                     const asharia::RenderGraphDiagnosticsSnapshot& snapshot) {
        const std::string title = "Capture: frame " + std::to_string(capture.frameIndex) +
                                  ", view " + viewportKindName(capture.viewKind) + ", " +
                                  std::to_string(capture.requestedExtent.width) + "x" +
                                  std::to_string(capture.requestedExtent.height);
        ImGui::TextUnformatted(title.c_str());
        const std::string state = "Frame Debug: " + std::string{frameDebugger.stateName()} +
                                  ", submitted epoch " +
                                  std::to_string(capture.submittedFrameEpoch);
...
namespace asharia::editor {
    ...
    void RenderGraphPanel::draw(EditorFrameContext& context, EditorPanelState& state) {
        ...
        const std::optional<EditorFrameDebugCapture>& activeCapture =
            pausedCapture ? pausedCapture : context.frameDebugger.latestCapture();
        context.frameDebugger.notifyRenderGraphPanelDrawn(activeCapture.has_value());
        if (!activeCapture) {
            ImGui::TextUnformatted("No captured RenderGraph snapshot.");
            return;
        }

        const asharia::RenderGraphDiagnosticsSnapshot& snapshot =
            activeCapture->diagnostics.renderGraph;
        drawSummary(context.frameDebugger, *activeCapture, snapshot);
        ImGui::Separator();

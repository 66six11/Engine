...
namespace {
    ...
    [[nodiscard]] EditorViewportNativeAbiHeader streamHandleV9Header() {
        return EditorViewportNativeAbiHeader{
            .abiVersion = EDITOR_NATIVE_ABI_VERSION,
            .structSize = static_cast<std::uint32_t>(sizeof(EditorViewportNativeStreamHandleV9)),
        };
    }

    [[nodiscard]] EditorViewportNativeAbiHeader readyFrameV9Header() {
        return EditorViewportNativeAbiHeader{
            .abiVersion = EDITOR_NATIVE_ABI_VERSION,
            .structSize = static_cast<std::uint32_t>(sizeof(EditorViewportNativeReadyFrameV9)),
        };
    }

    [[nodiscard]] EditorViewportNativeAbiHeader streamPollV9Header() {
        return EditorViewportNativeAbiHeader{
            .abiVersion = EDITOR_NATIVE_ABI_VERSION,
            .structSize = static_cast<std::uint32_t>(sizeof(EditorViewportNativeStreamPollV9)),
        };
    }
    ...
    [[nodiscard]] EditorViewportNativeAbiHeader runtimeStatsV9Header() {
        return EditorViewportNativeAbiHeader{
            .abiVersion = EDITOR_NATIVE_ABI_VERSION,
            .structSize = static_cast<std::uint32_t>(sizeof(EditorViewportNativeRuntimeStatsV9)),
        };
    }
    ...
    [[nodiscard]] bool
    hasSupportedPresentRequestV9Header(const EditorViewportNativePresentRequestV9& request) {
        return request.header.abiVersion == EDITOR_NATIVE_ABI_VERSION &&
               request.header.structSize >= sizeof(EditorViewportNativePresentRequestV9);
    }

    [[nodiscard]] asharia::editor::EditorExtent2D
    logicalExtentFor(const EditorViewportNativePresentRequestV9& request) {
    ...
    [[nodiscard]] bool
    gizmoTargetsSelection(const EditorViewportNativeTransformGizmoV9& gizmo,
    ...
    [[nodiscard]] bool
    validAuthoredMesh(const EditorViewportNativeAuthoredMeshSnapshotV9& snapshot) {
    ...
    [[nodiscard]] bool
    isZeroTransformGizmo(const EditorViewportNativeTransformGizmoV9& gizmo) noexcept {
    ...
    [[nodiscard]] bool
    validTransformGizmo(const EditorViewportNativeTransformGizmoV9& gizmo) noexcept {
        return hasValue(gizmo.objectId) && finite3(gizmo.position) &&
               gizmo.kind <= EditorViewportNativeTransformGizmoKind_Rotate &&
               gizmo.hoveredAxis <= EditorViewportNativeGizmoAxis_Z &&
               gizmo.activeAxis <= EditorViewportNativeGizmoAxis_Z;
    }

    [[nodiscard]] bool validPresentRequestV9(const EditorViewportNativePresentRequestV9& request) {
        ...
        constexpr std::uint32_t kKnownFlags =
            EditorViewportNativePresentRequestV9Flags_HasLogicalExtent |
            EditorViewportNativePresentRequestV9Flags_FlashSentinelCorners |
            EditorViewportNativePresentRequestV9Flags_CaptureSceneMeshEvidence |
            EditorViewportNativePresentRequestV9Flags_HasSelectionOutline |
            EditorViewportNativePresentRequestV9Flags_HasTransformGizmo;
        const bool hasSelectionOutline =
            (request.flags & EditorViewportNativePresentRequestV9Flags_HasSelectionOutline) != 0U;
        const bool hasTransformGizmo =
            (request.flags & EditorViewportNativePresentRequestV9Flags_HasTransformGizmo) != 0U;
        ...
        if (!hasValue(request.sessionId) || !hasValue(request.targetId) ||
            ...
            (request.flags & EditorViewportNativePresentRequestV9Flags_HasLogicalExtent) == 0U ||
        ...
        const std::span<const EditorViewportNativeAuthoredMeshSnapshotV9> authoredMeshes{
    ...
    [[nodiscard]] bool
    hasUniqueAuthoredMeshIdentitiesV9(const EditorViewportNativePresentRequestV9& request) {
        ...
        for (const EditorViewportNativeAuthoredMeshSnapshotV9& mesh :
    ...
    void clearStreamHandleV9(EditorViewportNativeStreamHandleV9* stream, std::uint32_t status) {
        if (stream == nullptr) {
            return;
        }
        *stream = EditorViewportNativeStreamHandleV9{
            .header = streamHandleV9Header(),
            .status = status,
            .capabilities = EditorViewportNativeStreamCapabilitiesV9_None,
            .streamId = 0U,
        };
    }

    void clearReadyFrameV9(EditorViewportNativeReadyFrameV9* frame, std::uint32_t status) {
        if (frame == nullptr) {
            return;
        }
        *frame = {};
        frame->header = readyFrameV9Header();
        frame->status = status;
    }

    void clearStreamPollV9(EditorViewportNativeStreamPollV9* poll, std::uint32_t status) {
        if (poll == nullptr) {
            return;
        }
        *poll = {};
        poll->header = streamPollV9Header();
        poll->status = status;
    ...
    [[nodiscard]] std::uint32_t
    submitStreamFrameV9(std::uint64_t streamId,
                        const EditorViewportNativePresentRequestV9& request) {
        ...
        try {
            ...
            if (request.authoredMeshCount != 0U) {
                const std::span<const EditorViewportNativeAuthoredMeshSnapshotV9> requestMeshes{
                    request.authoredMeshes, request.authoredMeshCount};
                for (const EditorViewportNativeAuthoredMeshSnapshotV9& mesh : requestMeshes) {
        ...
        const asharia::editor::EditorSharedViewportPresentDesc desc{
            .panelId = "viewport-stream/native-v9",
            ...
            .captureSceneMeshEvidence =
                (request.flags &
                 EditorViewportNativePresentRequestV9Flags_CaptureSceneMeshEvidence) != 0U,
            .flashSentinelCorners =
                (request.flags & EditorViewportNativePresentRequestV9Flags_FlashSentinelCorners) !=
                0U,
            .hasSelectionOutline =
                (request.flags & EditorViewportNativePresentRequestV9Flags_HasSelectionOutline) !=
                0U,
    ...
    [[nodiscard]] std::uint32_t
    openStreamV9(const EditorViewportNativeCompatibilityRequest* compatibility,
        ...
        if (compatibility == nullptr || stream == nullptr) {
            clearStreamHandleV9(stream, EditorViewportNativeStatus_InvalidArgument);
            return EditorViewportNativeStatus_InvalidArgument;
        }
        if (!hasSupportedRequestHeader(*compatibility)) {
            clearStreamHandleV9(stream, EditorViewportNativeStatus_UnsupportedAbi);
            return EditorViewportNativeStatus_UnsupportedAbi;
        }
        if (!hasSupportedHandleTypes(*compatibility)) {
            clearStreamHandleV9(stream, EditorViewportNativeStatus_UnsupportedHandleType);
        ...
        try {
            ...
            if (!deviceSnapshot) {
                clearStreamHandleV9(stream, EditorViewportNativeStatus_Unavailable);
                return EditorViewportNativeStatus_Unavailable;
            }
            if (!matchesRequestedDevice(*compatibility, deviceSnapshot->identity)) {
                clearStreamHandleV9(stream, EditorViewportNativeStatus_DeviceMismatch);
            ...
            *stream = EditorViewportNativeStreamHandleV9{
                ...
                .capabilities = supportsWireframe
                                    ? EditorViewportNativeStreamCapabilitiesV9_Wireframe
                                    : EditorViewportNativeStreamCapabilitiesV9_None,
                .streamId = *opened,
            };
            return EditorViewportNativeStatus_Success;
        } catch (...) {
            clearStreamHandleV9(stream, EditorViewportNativeStatus_InternalError);
            return EditorViewportNativeStatus_InternalError;
        }
    }
...
editor_viewport_open_stream_v9(const EditorViewportNativeCompatibilityRequest* compatibility,
                               EditorViewportNativeStreamHandleV9* stream) {
    return openStreamV9(compatibility, stream);
}

#if defined(ASHARIA_EDITOR_NATIVE_TESTING)
std::uint32_t EDITOR_NATIVE_CALL editor_viewport_open_stream_v9_for_test(
    const EditorViewportNativeCompatibilityRequest* compatibility, std::uint32_t capabilities,
    EditorViewportNativeStreamHandleV9* stream) {
    constexpr std::uint32_t kKnownCapabilities = EditorViewportNativeStreamCapabilitiesV9_Wireframe;
    if ((capabilities & ~kKnownCapabilities) != 0U) {
        clearStreamHandleV9(stream, EditorViewportNativeStatus_InvalidArgument);
        return EditorViewportNativeStatus_InvalidArgument;
    }
    return openStreamV9(compatibility, stream,
                        (capabilities & EditorViewportNativeStreamCapabilitiesV9_Wireframe) != 0U);
}
#endif

std::uint32_t EDITOR_NATIVE_CALL editor_viewport_submit_latest_v9(
    std::uint64_t streamId, const EditorViewportNativePresentRequestV9* request) {
    if (streamId == 0U || request == nullptr) {
        return EditorViewportNativeStatus_InvalidArgument;
    }
    if (!hasSupportedPresentRequestV9Header(*request)) {
        return EditorViewportNativeStatus_UnsupportedAbi;
    }
    if (!validPresentRequestV9(*request)) {
        return EditorViewportNativeStatus_InvalidArgument;
    }
    try {
        if (!hasUniqueAuthoredMeshIdentitiesV9(*request)) {
            return EditorViewportNativeStatus_InvalidArgument;
        }
        return submitStreamFrameV9(streamId, *request);
    } catch (...) {
        return EditorViewportNativeStatus_InternalError;
    }
}
...
editor_viewport_try_take_ready_v9(std::uint64_t streamId, EditorViewportNativeReadyFrameV9* frame) {
    if (streamId == 0U || frame == nullptr) {
        clearReadyFrameV9(frame, EditorViewportNativeStatus_InvalidArgument);
        return EditorViewportNativeStatus_InvalidArgument;
    }
    ...
    try {
        ...
        if (!ready) {
            clearReadyFrameV9(frame, EditorViewportNativeStatus_Unavailable);
            return EditorViewportNativeStatus_Unavailable;
        }
        if (!*ready) {
            clearReadyFrameV9(frame, EditorViewportNativeStatus_Success);
            return EditorViewportNativeStatus_Success;
        }
        ...
        if (format == EditorViewportNativeImageFormat_Unknown) {
            ...
            clearReadyFrameV9(frame, EditorViewportNativeStatus_RenderFailed);
            return EditorViewportNativeStatus_RenderFailed;
        }

        *frame = EditorViewportNativeReadyFrameV9{
            .header = readyFrameV9Header(),
            .status = EditorViewportNativeStatus_Success,
            .hasFrame = 1U,
            .streamId = streamId,
            ...
            .sceneMeshReceipt =
                EditorViewportNativeSceneMeshReceiptV9{
        ...
        clearReadyFrameV9(frame, EditorViewportNativeStatus_InternalError);
        return EditorViewportNativeStatus_InternalError;
    }
}

std::uint32_t EDITOR_NATIVE_CALL editor_viewport_complete_frame_v9(std::uint64_t streamId,
                                                                   void* nativeSlot,
...
std::uint32_t EDITOR_NATIVE_CALL editor_viewport_release_slot_import_v9(std::uint64_t streamId,
                                                                        void* nativeSlot) {
    try {
...
std::uint32_t EDITOR_NATIVE_CALL editor_viewport_close_stream_v9(std::uint64_t streamId) {
    try {
        auto closed =
...
editor_viewport_poll_stream_v9(std::uint64_t streamId, EditorViewportNativeStreamPollV9* poll) {
    if (streamId == 0U || poll == nullptr) {
        clearStreamPollV9(poll, EditorViewportNativeStatus_InvalidArgument);
        return EditorViewportNativeStatus_InvalidArgument;
    }
    try {
        auto snapshot =
        ...
        if (!snapshot) {
            clearStreamPollV9(poll, EditorViewportNativeStatus_Unavailable);
            return EditorViewportNativeStatus_Unavailable;
        }
        *poll = EditorViewportNativeStreamPollV9{
            .header = streamPollV9Header(),
            .status = EditorViewportNativeStatus_Success,
        ...
        clearStreamPollV9(poll, EditorViewportNativeStatus_InternalError);
        return EditorViewportNativeStatus_InternalError;
    }
}

std::uint32_t EDITOR_NATIVE_CALL editor_viewport_destroy_stream_v9(std::uint64_t streamId) {
    try {
        auto destroyed =
...
editor_viewport_query_runtime_stats_v9(EditorViewportNativeRuntimeStatsV9* stats) {
    ...
    const asharia::editor::EditorSharedViewportRuntimeStats runtimeStats =
        asharia::editor::EditorSharedViewportRuntime::instance().stats();
    *stats = EditorViewportNativeRuntimeStatsV9{
        .header = runtimeStatsV9Header(),
        .framesRendered = runtimeStats.framesRendered,
        .producersCreated = runtimeStats.producersCreated,
        .packetsCreated = runtimeStats.packetsCreated,
        .outstandingPackets = static_cast<std::uint64_t>(runtimeStats.outstandingPackets),

...
namespace asharia::editor {
    namespace {
        ...
        [[nodiscard]] EditorViewportNativePresentRequestV10
        makeFrameRequest(std::uint64_t requestSequence) {
            return EditorViewportNativePresentRequestV10{
                .header =
                    EditorViewportNativeAbiHeader{
                        .abiVersion = EDITOR_NATIVE_ABI_VERSION,
                        .structSize = static_cast<std::uint32_t>(
                            sizeof(EditorViewportNativePresentRequestV10)),
                    },
                ...
                .widthPixels = 384U,
                .heightPixels = 224U,
                .flags = EditorViewportNativePresentRequestV10Flags_HasLogicalExtent,
                .camera =
        ...
        [[nodiscard]] bool waitForReadyFrame(std::uint64_t streamId,
                                             EditorViewportNativeReadyFrameV10& frame) {
            const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds{5};
            while (std::chrono::steady_clock::now() < deadline) {
                if (editor_viewport_try_take_ready_v10(streamId, &frame) !=
        ...
        [[nodiscard]] bool waitForPoll(std::uint64_t streamId,
                                       EditorViewportNativeStreamPollV10& poll,
                                       std::uint32_t lifecycle) {
            const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds{5};
            while (std::chrono::steady_clock::now() < deadline) {
                if (editor_viewport_poll_stream_v10(streamId, &poll) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
        ...
        [[nodiscard]] bool completeNotSubmitted(std::uint64_t streamId, void* nativeSlot) {
            return editor_viewport_complete_frame_v10(
                       streamId, nativeSlot,
        ...
        [[nodiscard]] bool smokeUnsupportedWireframeRecovery() {
            ...
            if (editor_viewport_open_stream_v10_for_test(
                    &compatibility, EditorViewportNativeStreamCapabilitiesV10_None, &stream) !=
                    EditorViewportNativeStatus_Success ||
                stream.status != EditorViewportNativeStatus_Success || stream.streamId == 0U ||
                stream.capabilities != EditorViewportNativeStreamCapabilitiesV10_None) {
                logError("Viewport V10 unsupported-wireframe smoke could not open a stream.");
                return false;
            }
            const std::uint64_t streamId = stream.streamId;

            EditorViewportNativePresentRequestV10 wireframeRequest = makeFrameRequest(901U);
            wireframeRequest.sceneRasterMode = EditorViewportNativeSceneRasterMode_Wireframe;
            if (editor_viewport_submit_latest_v10(streamId, &wireframeRequest) !=
                EditorViewportNativeStatus_FeatureUnavailable) {
                logError("Viewport V10 did not reject unsupported wireframe precisely.");
                return false;
            }

            EditorViewportNativeStreamPollV10 rejectedPoll{};
            if (editor_viewport_poll_stream_v10(streamId, &rejectedPoll) !=
                    EditorViewportNativeStatus_Success ||
                ...
                logError("Unsupported wireframe mutated or faulted the V10 stream.");
                return false;
            }

            EditorViewportNativePresentRequestV10 solidRequest = makeFrameRequest(902U);
            if (editor_viewport_submit_latest_v10(streamId, &solidRequest) !=
                EditorViewportNativeStatus_Success) {
                logError("Viewport V10 did not recover from wireframe rejection with Solid.");
                return false;
            }
            EditorViewportNativeReadyFrameV10 solidFrame{};
            if (!waitForReadyFrame(streamId, solidFrame) ||
                ...
                editor_viewport_release_slot_import_v10(streamId, solidFrame.nativeSlot) !=
                    EditorViewportNativeStatus_Success ||
                editor_viewport_close_stream_v10(streamId) != EditorViewportNativeStatus_Success) {
                logError("Viewport V10 Solid recovery did not complete cleanly.");
                return false;
            }
            EditorViewportNativeStreamPollV10 closedPoll{};
            if (!waitForPoll(streamId, closedPoll, EditorViewportNativeStreamLifecycle_Closed) ||
                ...
                editor_viewport_destroy_stream_v10(streamId) !=
                    EditorViewportNativeStatus_Success) {
                logError("Viewport V10 Solid recovery stream did not close cleanly.");
                return false;
            }
            return true;
        }
        ...
        [[nodiscard]] bool smokeBoundedLatestWinsStream() {
            EditorViewportNativeCompatibilityRequest compatibility = makeCompatibilityRequest();
            EditorViewportNativeStreamHandleV10 stream{};
            if (editor_viewport_open_stream_v10(&compatibility, &stream) !=
                    EditorViewportNativeStatus_Success ||
                stream.status != EditorViewportNativeStatus_Success || stream.streamId == 0U) {
                logError("Viewport V10 smoke could not open a stream.");
                return false;
            }

            EditorViewportNativePresentRequestV10 firstRequest = makeFrameRequest(1U);
            EditorViewportNativePresentRequestV10 invalidAxisRequest = firstRequest;
            invalidAxisRequest.camera.fieldOfViewAxis = 2U;
            EditorViewportNativePresentRequestV10 zeroFieldOfViewRequest = firstRequest;
            zeroFieldOfViewRequest.camera.fieldOfViewRadians = 0.0F;
            EditorViewportNativePresentRequestV10 excessiveFieldOfViewRequest = firstRequest;
            excessiveFieldOfViewRequest.camera.fieldOfViewRadians = std::numbers::pi_v<float>;
            if (editor_viewport_submit_latest_v10(stream.streamId, &invalidAxisRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId, &zeroFieldOfViewRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId, &excessiveFieldOfViewRequest) !=
                    EditorViewportNativeStatus_InvalidArgument) {
                logError("Viewport V10 smoke accepted an invalid camera projection contract.");
                return false;
            }
            const std::array<EditorViewportNativeAuthoredMeshSnapshotV10, 1> authoredMeshes{
                EditorViewportNativeAuthoredMeshSnapshotV10{
            ...
            firstRequest.flags |=
                EditorViewportNativePresentRequestV10Flags_CaptureSceneMeshEvidence;
            firstRequest.flags |= EditorViewportNativePresentRequestV10Flags_HasSelectionOutline;
            firstRequest.flags |= EditorViewportNativePresentRequestV10Flags_HasTransformGizmo;
            std::ranges::copy(authoredMeshes.front().objectId, firstRequest.selectedObjectId);
            firstRequest.viewStateRevision = 71U;
            firstRequest.transformGizmo = EditorViewportNativeTransformGizmoV10{
            ...
            EditorViewportNativePresentRequestV10 invalidGizmoAxisRequest = firstRequest;
            invalidGizmoAxisRequest.transformGizmo.activeAxis = 4U;
            EditorViewportNativePresentRequestV10 invalidGizmoKindRequest = firstRequest;
            invalidGizmoKindRequest.transformGizmo.kind = 3U;
            EditorViewportNativePresentRequestV10 invalidGizmoRotationRequest = firstRequest;
            invalidGizmoRotationRequest.transformGizmo.rotation[3] = 2.0F;
            EditorViewportNativePresentRequestV10 mismatchedGizmoSelectionRequest = firstRequest;
            mismatchedGizmoSelectionRequest.transformGizmo.objectId.high += 1U;
            EditorViewportNativePresentRequestV10 unflaggedGizmoRequest = firstRequest;
            unflaggedGizmoRequest.flags &=
                ~EditorViewportNativePresentRequestV10Flags_HasTransformGizmo;
            EditorViewportNativePresentRequestV10 orphanGizmoRequest = makeFrameRequest(1U);
            orphanGizmoRequest.flags |=
                EditorViewportNativePresentRequestV10Flags_HasTransformGizmo;
            orphanGizmoRequest.transformGizmo = firstRequest.transformGizmo;
            if (editor_viewport_submit_latest_v10(stream.streamId, &invalidGizmoAxisRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId,
                                                  &mismatchedGizmoSelectionRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId, &invalidGizmoKindRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId, &invalidGizmoRotationRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId, &unflaggedGizmoRequest) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                editor_viewport_submit_latest_v10(stream.streamId, &orphanGizmoRequest) !=
                    EditorViewportNativeStatus_InvalidArgument) {
                logError("Viewport V10 smoke accepted an invalid Transform Gizmo contract.");
                return false;
            }
            ...
            EditorViewportNativePresentRequestV10 duplicateObjectRequest = firstRequest;
            ...
            if (editor_viewport_submit_latest_v10(stream.streamId, &duplicateObjectRequest) !=
                EditorViewportNativeStatus_InvalidArgument) {
                logError("Viewport V10 smoke accepted duplicate scene object mesh identities.");
                return false;
            }
            ...
            EditorViewportNativePresentRequestV10 duplicateEntityRequest = firstRequest;
            ...
            if (editor_viewport_submit_latest_v10(stream.streamId, &duplicateEntityRequest) !=
                EditorViewportNativeStatus_InvalidArgument) {
                logError("Viewport V10 smoke accepted duplicate runtime entity mesh identities.");
                return false;
            }

            EditorViewportNativeStreamPollV10 rejectedIdentityPoll{};
            if (editor_viewport_poll_stream_v10(stream.streamId, &rejectedIdentityPoll) !=
                    EditorViewportNativeStatus_Success ||
                ...
                logError("Rejected V10 requests mutated the stream state.");
                return false;
            }

            if (editor_viewport_submit_latest_v10(stream.streamId, &firstRequest) !=
                EditorViewportNativeStatus_Success) {
                return false;
            }
            const auto firstReadyDeadline =
                std::chrono::steady_clock::now() + std::chrono::seconds{5};
            EditorViewportNativeStreamPollV10 firstReadyPoll{};
            while (std::chrono::steady_clock::now() < firstReadyDeadline) {
                if (editor_viewport_poll_stream_v10(stream.streamId, &firstReadyPoll) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
            ...
            if (firstReadyPoll.hasReadyFrame == 0U) {
                logError("Viewport V10 smoke did not render its first ready frame.");
                return false;
            }
            ...
            for (std::uint64_t sequence = 2U; sequence <= 32U; ++sequence) {
                EditorViewportNativePresentRequestV10 request = makeFrameRequest(sequence);
                if (editor_viewport_submit_latest_v10(stream.streamId, &request) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
            }
            EditorViewportNativeStreamPollV10 burstPoll{};
            if (editor_viewport_poll_stream_v10(stream.streamId, &burstPoll) !=
                    EditorViewportNativeStatus_Success ||
                burstPoll.hasPendingLatest == 0U || burstPoll.coalescedRequests < 30U) {
                logError("Viewport V10 smoke did not coalesce its pending-latest burst.");
                return false;
            }
            ...
            EditorViewportNativeReadyFrameV10 firstFrame{};
            if (!waitForReadyFrame(stream.streamId, firstFrame) ||
                ...
                logError("Viewport V10 smoke did not receive its first frame.");
                return false;
            }
            const EditorViewportNativeSceneMeshReceiptV10& receipt = firstFrame.sceneMeshReceipt;
            if (receipt.inputCount != 1U || receipt.resolvedCount != 1U ||
                ...
                logError("Viewport V10 smoke did not publish the authored scene mesh receipt.");
                return false;
            }

            EditorViewportNativeReadyFrameV10 secondFrame{};
            if (!waitForReadyFrame(stream.streamId, secondFrame) ||
                ...
                logError("Viewport V10 smoke did not publish the latest burst request on slot 2.");
                return false;
            }

            EditorViewportNativePresentRequestV10 thirdRequest = makeFrameRequest(33U);
            if (editor_viewport_submit_latest_v10(stream.streamId, &thirdRequest) !=
                EditorViewportNativeStatus_Success) {
                return false;
            }
            EditorViewportNativeReadyFrameV10 thirdFrame{};
            if (!waitForReadyFrame(stream.streamId, thirdFrame) ||
                ...
                logError("Viewport V10 smoke did not allocate its third bounded slot.");
                return false;
            }

            EditorViewportNativePresentRequestV10 fourthRequest = makeFrameRequest(34U);
            if (editor_viewport_submit_latest_v10(stream.streamId, &fourthRequest) !=
                EditorViewportNativeStatus_Success) {
                return false;
            }
            EditorViewportNativeStreamPollV10 boundedPoll{};
            if (editor_viewport_poll_stream_v10(stream.streamId, &boundedPoll) !=
                    EditorViewportNativeStatus_Success ||
                boundedPoll.slotCount != 3U || boundedPoll.hasPendingLatest == 0U ||
                boundedPoll.hasReadyFrame != 0U) {
                logError("Viewport V10 smoke exceeded or bypassed its three-slot bound.");
                return false;
            }

            if (editor_viewport_complete_frame_v10(stream.streamId, firstFrame.nativeSlot, 99U) !=
                    EditorViewportNativeStatus_InvalidArgument ||
                !completeNotSubmitted(stream.streamId, firstFrame.nativeSlot)) {
                logError("Viewport V10 smoke did not preserve ownership after invalid completion.");
                return false;
            }
            EditorViewportNativeStreamPollV10 retainedPoll{};
            if (editor_viewport_poll_stream_v10(stream.streamId, &retainedPoll) !=
                    EditorViewportNativeStatus_Success ||
                retainedPoll.hasPendingLatest == 0U || retainedPoll.hasReadyFrame != 0U) {
                logError("Viewport V10 smoke reused the compositor's sole available slot.");
                return false;
            }
            if (!completeNotSubmitted(stream.streamId, secondFrame.nativeSlot)) {
                return false;
            }
            EditorViewportNativeReadyFrameV10 fourthFrame{};
            if (!waitForReadyFrame(stream.streamId, fourthFrame) ||
                ...
                logError("Viewport V10 smoke did not reuse the completed persistent slot.");
                return false;
            }
            ...
            for (void* nativeSlot : importedSlots) {
                if (editor_viewport_release_slot_import_v10(stream.streamId, nativeSlot) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
            }
            if (editor_viewport_close_stream_v10(stream.streamId) !=
                EditorViewportNativeStatus_Success) {
                return false;
            }
            EditorViewportNativeStreamPollV10 closedPoll{};
            if (!waitForPoll(stream.streamId, closedPoll,
                ...
                editor_viewport_destroy_stream_v10(stream.streamId) !=
                    EditorViewportNativeStatus_Success) {
                logError("Viewport V10 smoke did not close and destroy its stream.");
                return false;
            }
            return true;
        }
        ...
        [[nodiscard]] bool smokeFourStreamColdStartFairness() {
            ...
            std::array<EditorViewportNativeStreamHandleV10, kStreamCount> streams{};
            EditorViewportNativeCompatibilityRequest compatibility = makeCompatibilityRequest();
            for (EditorViewportNativeStreamHandleV10& stream : streams) {
                if (editor_viewport_open_stream_v10(&compatibility, &stream) !=
                        EditorViewportNativeStatus_Success ||
                    stream.status != EditorViewportNativeStatus_Success || stream.streamId == 0U) {
            ...
            EditorViewportNativePresentRequestV10 first = makeFrameRequest(kRealtimeFirstSequence);
            if (editor_viewport_submit_latest_v10(streams.at(0).streamId, &first) !=
                EditorViewportNativeStatus_Success) {
                return false;
            }
            EditorViewportNativeStreamPollV10 realtimeReady{};
            ...
            for (std::uint64_t sequence = kRealtimeFirstSequence + 1U;
                ...
                EditorViewportNativePresentRequestV10 request = makeFrameRequest(sequence);
                if (editor_viewport_submit_latest_v10(streams.at(0).streamId, &request) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
            }
            ...
            for (std::size_t index = 1U; index < streams.size(); ++index) {
                EditorViewportNativePresentRequestV10 request =
                    makeFrameRequest(expectedSequences.at(index));
                request.targetId.low += static_cast<std::uint64_t>(index);
                if (editor_viewport_submit_latest_v10(streams.at(index).streamId, &request) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
            }
            ...
            EditorViewportNativeStreamPollV10 realtimePoll{};
            if (editor_viewport_poll_stream_v10(streams.at(0).streamId, &realtimePoll) !=
                    EditorViewportNativeStatus_Success ||
            ...
            for (std::size_t index = 0U; index < streams.size(); ++index) {
                ...
                if (!waitForReadyFrame(streams.at(index).streamId, frame) ||
                    ...
                    editor_viewport_release_slot_import_v10(streams.at(index).streamId,
                                                            frame.nativeSlot) !=
                        EditorViewportNativeStatus_Success) {
            ...
            for (std::size_t index = 1U; index < streams.size(); ++index) {
                if (editor_viewport_close_stream_v10(streams.at(index).streamId) !=
                    EditorViewportNativeStatus_Success) {
                    return false;
                }
            }
            for (std::size_t index = 1U; index < streams.size(); ++index) {
                ...
                if (!waitForPoll(streams.at(index).streamId, closedPoll,
                    ...
                    editor_viewport_destroy_stream_v10(streams.at(index).streamId) !=
                        EditorViewportNativeStatus_Success) {
                    logError("Viewport fairness smoke let pending render work starve close.");
                    return false;
                }
            }
            if (editor_viewport_close_stream_v10(streams.at(0).streamId) !=
                EditorViewportNativeStatus_Success) {
                return false;
            }
            EditorViewportNativeStreamPollV10 realtimeClosedPoll{};
            if (!waitForPoll(streams.at(0).streamId, realtimeClosedPoll,
                ...
                editor_viewport_destroy_stream_v10(streams.at(0).streamId) !=
                    EditorViewportNativeStatus_Success) {
    ...
    bool runViewportNativeBridgeSmoke() {
        ...
        if (editor_viewport_query_render_thread_stats(&stats) !=
            ...
            logError("Viewport V10 smoke did not stop and join its native render thread.");
            return false;
        }
        return true;
    }

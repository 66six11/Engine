...
41. Transaction service v0 提供 UI-neutral `IEditorTransactionService`、edit command descriptor、begin/commit/rollback/undo/redo、dirty-state snapshot 和 diagnostics；它只管理编辑器命令历史，不写 Transform、不查询真实 runtime scene，也不接 native ABI
```

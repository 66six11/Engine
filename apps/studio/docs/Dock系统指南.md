...
37. Scene snapshot provider refresh seam remains read-only: `InMemorySceneSnapshotProvider.ReplaceSnapshot(SceneSnapshot)` can publish replacement snapshots and raise `SnapshotChanged`, but this does not write Transform data and does not query a native runtime scene.
```

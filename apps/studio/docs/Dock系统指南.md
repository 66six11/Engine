...
28. `Window/Panels/*` 菜单从 `PanelDescriptor` 生成，打开时先激活已有 panel，再按默认区域重开
...
Feature 只注册 `PanelDescriptor`，不要直接创建 Dock 控件或 View：
...
4. Panel command registry：把 `Window/Panels/*` 从当前 descriptor 菜单推进到统一命令/快捷键注册。

...
47. Panel instance manager v0 owns built-in panel content creation, logical panel attach/detach callbacks, and deterministic close/reset/shutdown release, including floating workspace host close. `KeepAlive` close detaches the tab but keeps content until workspace/session disposal; `RecreateOnOpen` close and reset dispose the old content before reopening.
48. Panel lifecycle callbacks v0 expose `IEditorPanelLifecycleSink` with attached/activated/deactivated/detached notifications and a UI-neutral `EditorPanelLifecycleContext`. `PanelInstanceManager` owns attach/detach and `EditorDockWorkspaceViewModel` owns active/deactive transitions. This is still editor-framework scheduling only: provider reload, plugin reload, native viewport ownership, renderer FPS and engine simulation remain deferred.
49. Panel frame update scheduler v0 exposes `IEditorPanelFrameUpdateSink`, `EditorPanelFrameUpdateRequest`, `EditorPanelFrameContext` and `EditorPanelFrameScheduler`. Panels opt into manual, visible or active-only ticks with optional target FPS throttling; tests drive the scheduler through explicit `Tick(now)`. `RequestRepaint()` records an editor repaint request on the frame context only; it does not call Avalonia render APIs, native viewport code, renderer presentation, or engine simulation.
50. Provider contribution v0 registers the fixture-backed active scene provider under `scene.active` through `EditorProviderHost`. `ISceneSnapshotProvider` remains a read-only data contract; provider reload, native bridge connection, script VM, external plugins, Console/Problems projection and writable scene editing remain deferred.

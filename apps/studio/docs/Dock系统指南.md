...
51. Process diagnostics/log ingress v1 uses the one App-owned `IStudioDiagnosticHub`: count + UTF-8 payload bounded diagnostic/log rings, bounded subscribers, cursor/drop/truncation evidence and explicit managed/native/framework/subprocess mapping. #381/#383 opens or focuses one `Diagnostics` panel through `Window/Panels/Diagnostics`; its Console and Problems tabs share one panel projection owner but use separate log/diagnostic invalidation subscriptions. The panel is `KeepAlive`: close/float/reopen reuses the same content and both subscriptions, while terminal workspace/Shell dispose releases both. The subprocess contract is not production-wired because Studio owns no subprocess; shell command input/CVar, external plugins, persistence, report/crash, arbitrary RPC and remote control remain deferred.
```

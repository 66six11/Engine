...
31. `WorkbenchActionExecutor` 是当前 action execution contract，`OpenPanel` action 通过 `PanelCommandService.OpenOrFocusPanel` 执行
32. Command Palette 可通过 `Tools > Command Palette` 或 `Ctrl+Shift+P` 打开；当前只是固定入口，不是完整 shortcut router
...
35. Hierarchy demo data source v0 通过 `IHierarchyDataSource` 提供小规模可选节点，用于当前 UI 验证和 selection-to-inspector 数据流测试；后续真实场景数据源替换该 provider
```
...
4. 完整快捷键策略、更多 action kind 和完整命令系统
...
5. Hierarchy provider follow-up：用真实 scene object provider 替换 demo data source，并保留 `IEditorSelectionService` 作为面板同步边界。
6. Inspector provider follow-up：接入真实 scene object / asset provider，并扩展只读属性来源；编辑器控件和写回另做独立切片。

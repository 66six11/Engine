...
28. `Window/Panels/*` 菜单从 `WorkbenchActionDescriptor` 生成，panel action 执行时先激活已有 panel，再按默认区域重开
...
Execute  OpenPanel action 复用 PanelCommandService.OpenOrFocusPanel
...
4. Command palette follow-up：增加快捷键入口、分组和更多 action kind，但暂不做插件命令 API。

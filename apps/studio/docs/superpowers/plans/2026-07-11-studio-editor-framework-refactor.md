...
Selection checkpoint: `EditorSelectionService` now compiles from `Asharia.Studio.Application.Selection`; its six behavior tests compile from `Asharia.Studio.Application.Tests`, legacy consumers reference the Application implementation, and an architecture gate rejects a restored `Shell/Selection` owner.

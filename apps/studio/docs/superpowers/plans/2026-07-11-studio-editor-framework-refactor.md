...
Service checkpoints: `EditorSelectionService` compiles from `Asharia.Studio.Application.Selection`, `EditorBackgroundTaskService` from `Asharia.Studio.Application.Tasks`, `EditorLifecycleEventService` from `Asharia.Studio.Application.Lifecycle`, `EditorTransactionService` from `Asharia.Studio.Application.Transactions`, `EditorDiagnosticService` from `Asharia.Studio.Application.Diagnostics`, `ViewportScheduler` from `Asharia.Studio.Application.Viewports`, and `EditorPanelFrameScheduler` from `Asharia.Studio.Application.Panels`. Their focused behavior tests compile from `Asharia.Studio.Application.Tests`; legacy consumers reference the Application implementations where a production consumer exists, and architecture gates enforce Application assembly ownership without relying on legacy physical directories.

Command checkpoint: `EditorCommandStatusMessageRouter` compiles from `Asharia.Studio.Application.Commands` and decorates the existing public `IEditorGuiCommandExecutor`; the duplicate legacy `IWorkbenchCommandRouter` interface was deleted. The legacy descriptor-backed `WorkbenchCommandRouter` now implements the public executor contract, while action registry/handler execution, Dock panel commands and Avalonia shortcut parsing remain compatibility/Presentation-owned pending their own stable boundaries.

Command decorator validation checkpoint: Microsoft describes a decorator as an object that both contains another object and presents the same interface, matching the public-executor wrapper used here. Microsoft also specifies that uncaught delegate exceptions propagate to the caller and stop the remaining invocation sequence. Accordingly, status publication occurs only after the inner executor returns an `EditorCommandExecutionResult`; the decorator neither fabricates a result nor catches or translates an inner exception. See [Stream Decorator](https://learn.microsoft.com/en-us/archive/msdn-magazine/2005/september/net-matters-stream-decorator-single-instance-apps) and the C# [delegate invocation specification](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/language-specification/delegates#216-delegate-invocation).

UI dispatcher contract checkpoint: `IEditorUiDispatcher` compiles from `Asharia.Editor.Threading` and no longer has a legacy `Core/Abstractions` duplicate. Avalonia's official dispatcher contract defines `CheckAccess` as testing whether the caller is on the associated dispatcher thread and `Post` as scheduling work on that thread; the public editor contract intentionally preserves only those two portable operations. `AvaloniaEditorUiDispatcher` remains Presentation/legacy-owned, while `ImmediateEditorUiDispatcher` remains a synchronous compatibility default instead of being promoted to Application policy. See Avalonia [`IDispatcher`](https://api-docs.avaloniaui.net/docs/T_Avalonia_Threading_IDispatcher) and [threading model](https://docs.avaloniaui.net/docs/app-development/threading).

External validation checkpoint: Microsoft [.NET clean architecture guidance](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures) supports keeping stable interfaces in an inward dependency layer and wiring outer implementations at the composition root. The [.NET event guidelines](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/event) support the existing post-state-change `EventHandler` notifications. The [.NET thread-safe collections guidance](https://learn.microsoft.com/en-us/dotnet/standard/collections/thread-safe/) confirms that ordinary `List<T>`/`Dictionary<TKey,TValue>` state is not implicitly synchronized; therefore the migrated Transaction implementation remains serialized-context-only until a separate threading contract is approved.

Viewport timing validation checkpoint: Microsoft documents LINQ [`OrderBy`](https://learn.microsoft.com/en-us/dotnet/api/system.linq.enumerable.orderby?view=net-10.0) as stable; the scheduler additionally uses ordinal viewport ID as a deterministic tie-break. Microsoft documents [`TimeProvider.GetTimestamp()`](https://learn.microsoft.com/en-us/dotnet/api/system.timeprovider.gettimestamp?view=net-10.0) and [`Stopwatch`](https://learn.microsoft.com/en-us/dotnet/api/system.diagnostics.stopwatch?view=net-10.0) for accurate elapsed-time measurement. The current `DateTimeOffset` contract remains a diagnostic UTC-instant v0 and handles backward clock movement by rendering immediately, but production frame-loop cadence requires a future explicit monotonic timestamp contract rather than being inferred in this ownership move.

Panel frame threading validation checkpoint: Avalonia's [threading model](https://docs.avaloniaui.net/docs/app-development/threading) requires UI work on the owning dispatcher and recommends `DispatcherTimer` for periodic UI updates; Avalonia 12 uses the current dispatcher when constructing the timer. The legacy Window continues to create, start, stop, and subscribe that timer on its dispatcher. Application owns only the serial panel scheduling policy and public sink calls, not the timer, Window, Control, dispatcher, or a cross-thread execution guarantee.

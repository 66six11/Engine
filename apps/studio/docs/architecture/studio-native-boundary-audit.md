...
状态：Current audit（SceneDocument ABI v3、catalog query ABI v1 consumer 与 V10 async viewport stream 已建立；device epoch/recovery 仍开放）
...
V10 ABI 现在承担 production presentation consumer：显式 stream handle、immutable latest request、bounded full-slot
lease、异步 ready acquire 与 close/poll/destroy，并携带 authored mesh snapshots、per-view raster mode、Scene mesh receipt
与 optional discriminated Transform Gizmo packet；V10 Gizmo payload 增加 normalized object rotation，支持 renderer-owned
local-axis Scale handles 而不引入 scene/runtime pointer。
V1–V9 stream exports 均不导出，不提供 managed 或 deployment fallback；历史 runtime-stats diagnostics 版本链不是 stream
...
`EditorSharedViewportRuntime` 现在拥有一条 process-level RenderThread 和 V10 bounded latest-wins scheduler；每个 stream
...
当前普通 build 与 Release Editor Image 共同要求以下 V10 production exports：
...
失败。`msvc-debug-tests` 可以额外导出 `editor_viewport_open_stream_v10_for_test`，只用于显式 GPU/fault-injection 验收；普通本地
...
#359 后续由 V10 保留的当前事实：request 携 session/target/revision/sequence、真实 camera snapshot、view-local FOV axis 与最多 256 个来自
...
RenderThread 仍是 Vulkan queue 的唯一 consumer。V10 已按 stream 提供 pending-latest 覆盖和 ready-frame acquire；
...
1. **Managed Project Shell（已建立）**：Project 使用 dedicated ABI v2，Scene 使用 package-owned Document ABI v3；Viewport 使用 V10 async
...
4. **Viewport slice（V10 当前基线）**：`SceneDocument -> UI-neutral session -> camera/FOV axis/bounded Transform proxies
...
- 普通 Studio build 必须先重建所选 `StudioNativeBuildPreset` 的 `editor-native`，再由
  `ValidateStudioViewportNativeRuntimeContract` 对最终 `TargetDir` sibling 验证完整 V10 required exports 与 legacy V1--V9

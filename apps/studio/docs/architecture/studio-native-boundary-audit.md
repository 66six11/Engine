...
V7 ABI 现在承担 production presentation consumer：显式 stream handle、immutable latest request、bounded full-slot
lease、异步 ready acquire 与 close/poll/destroy，并携带 authored mesh snapshots、per-view raster mode 与 Scene mesh receipt。
V1–V6 stream exports 均不导出，不提供 managed 或 deployment fallback；历史 runtime-stats diagnostics 版本链不是 stream
...
`EditorSharedViewportRuntime` 现在拥有一条 process-level RenderThread 和 V7 bounded latest-wins scheduler；每个 stream
...
#359 后续由 V7 保留的当前事实：request 携 session/target/revision/sequence、真实 camera snapshot、view-local FOV axis 与最多 256 个来自
...
RenderThread 仍是 Vulkan queue 的唯一 consumer。V7 已按 stream 提供 pending-latest 覆盖和 ready-frame acquire；
...
4. **Viewport slice（V7 当前基线）**：`SceneDocument -> UI-neutral session -> camera/FOV axis/bounded Transform proxies

...
EngineBridge 当前提供 Scene World ABI v1 create/destroy、owner-thread deterministic disposal，以及复用 `Asharia.Runtime.EntityId` 的 entity create/destroy/is-alive；它不暴露 native pointer，不使用 finalizer thread 销毁 thread-affine World，也尚未提供 Transform/name、snapshot/query projection、ProjectSession wiring 或 native library deployment。

...
`Editor/**/*.cs` 原子生成 implicit SDK workspace；build/artifact pipeline 尚未落地。
...
或 workspace closure 漂移后 build-input current check 失败。它仍不 restore/build、不执行 MSBuild、不加载
candidate；`.asmdef`、Package/Avalonia resources、execution output 与 artifact inspection 是后继边界。
...
- Project Code 当前已落地 exact Editor Image、managed build environment inventory lease 与 Windows x64
  semantic build credential，以及 caller-bound 项目根 `Editor/**/*.cs` implicit SDK workspace；正式
  ProjectSession/manifest handoff、`.asmdef`、Package、build controller、artifact inspection 和 ALC pipeline
  尚未实现；

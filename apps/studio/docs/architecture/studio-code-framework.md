...
Task 4 的 service/state 迁移已开始：background task、diagnostic record/service、Frame Debug snapshot/provider、editing command、lifecycle event、selection、transaction、scene/world snapshot/provider interface，以及 backend-neutral Viewport identity/clock/render/scheduler/state contracts 已由 `Asharia.Editor` 唯一拥有；legacy service、Feature、ViewModel 和 Avalonia View 只消费这些公共合同。native Frame Debug payload/bridge、Viewport composition capability/native present transport、delegate-based `SceneProviderDescriptor`、provider registration/status 与 fixture provider implementation 仍在 compatibility Host；剩余 Panels/Commands family 尚未提升。

...
Viewport presentation proposal (Scene exact / Game fit / Frame Debug immutable capture)
  ...
  -> EngineBridge ViewportRenderStream (V8)
...
renderer-owned、按相机深度保持约 84 physical pixels 的世界空间 Translate Gizmo；hover/active 轴由显式 packet 着色，
...
presentation 不增加 runtime scene 字段或第二个 rotation truth；hint 的跨会话持久化、rotate/scale gizmo、snapping、
...
| `ViewportBridge` / `ViewportRenderStream` | V8 ABI 映射、typed status、view-local FOV axis、authored-mesh/Translate Gizmo snapshot、slot/frame lease 与 request-correlated mesh receipt | 调 Vulkan；猜测 stale native metadata；把 asset GUID 替换成 backend resource key |
...
```text
open_stream_v8(compatibility) -> streamId
submit_latest_v8(streamId, owning request snapshot)
try_take_ready_v8(streamId) -> optional self-describing frame
complete_frame_v8(streamId, slotId, completionKind)
release_slot_import_v8(streamId, slotId)
close_stream_v8(streamId)
poll_stream_v8(streamId)
destroy_stream_v8(streamId)
```

V1–V7 frame symbols 不导出，也没有 managed fallback。`query_composition_compatibility` 是一次性 device/handle control plane；
...
有界 authored mesh snapshots、per-view Scene raster mode 与可选 typed Translate Gizmo。Gizmo packet 只携 stable object identity、
world position 和 hover/active axis；缺少 Scene selection、非法 axis、non-finite position、flag/payload 不一致时整帧 fail closed。
...
空entity/当前不可呈现mesh提供诚实回退。fallback只遍历与V8 request相同的最多256个Transform debug proxies，并保留
...
选择路径本身不产生document mutation。单选 outline 与 Translate Gizmo 已由独立 view state 投影；hover outline、multi-select、
triangle-precise picking、通用asset bounds与GPU ID/readback均deferred。
...
字段。V8 保留双字段以维持自描述 ABI，但 Studio surface presentation 的硬约束是：
...
`MinimumPresentableSequence`。V8 immutable request 显式携带 revision、selected canonical UUID 与 presence flag；native ready frame
...
因此旧画面与当前相机不匹配时只能继续显示，不能把当前相机用于旧像素反投影；相机导航本身不新增 native/renderer 分支。
...
## 世界轴 Translate Gizmo MVP

#409 在上述 presented-frame 与 selection 门禁上增加单选、世界 X/Y/Z 轴平移，不引入 Physics 或通用工具框架。输入优先级固定为
`Alt` 相机导航 → 无修饰 LMB Gizmo → 普通 click selection。Application 捕获 stable `ObjectId`、起始 document revision、
authoritative Transform 与同帧 camera；screen-space 轴命中和 ray/plane 求交均为 UI-neutral 纯数学。视线平行轴、中心歧义、
stale frame/selection/document 或 non-finite 结果都 fail closed。

```text
presented interaction identity + selection
  -> hit one world axis
  -> pointer capture + transient latest-wins Transform preview
  -> V8 typed Gizmo packet + matching proxy/mesh preview Transform
...
pointer move 只设置 `GizmoChanged`，不推进 `MinimumPresentableSequence`，所以 240 Hz 输入不会把较慢 render/present cadence 的
每一帧都饿死；它也不逐 sample 写 `SceneDocument` 或制造数百条 Undo。松手前同步结束 active gesture，随后只提交一次 project edit；
...
V8 request 对匹配 object 的 debug proxy 与 authored mesh 使用同一预览 Transform，并携显式
`objectId + position + hoveredAxis + activeAxis`。native 只验证/copy owning packet；renderer producer 把 Gizmo 转成已有
`BasicDebugWorldLine`，按相机 depth 与垂直投影尺度保持约 84 physical pixels，普通 RGB、hover 黄色、active 白色；匹配对象原有
...
O3DE 3D Viewport 对 transform manipulator 的世界/局部 reference space 区分。本地第一片只选择 world translate，因为当前需求只证明
单选平移和一次性 ProjectSession history；拒绝复制 rotate/scale/plane handles、local/pivot modes、snapping、多选、通用 gizmo registry、
...
- 用 authoritative asset catalog/resource runtime 替换当前 validation-only product binding，并补 imported mesh hot-reload；V8 presentation ownership 不变；

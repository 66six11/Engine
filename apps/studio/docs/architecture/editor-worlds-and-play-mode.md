...
当前 Studio 已有 fixture-backed scene snapshot、Hierarchy/Inspector 只读投影、Scene View composition spike，以及 EngineBridge 的 owner-thread Scene World lifetime、entity create/destroy/is-alive 与 local Transform get/set baseline。尚无正式 project/document/world session、entity query/snapshot projection、transactional document mutation bridge、Play state machine 或 Game View。

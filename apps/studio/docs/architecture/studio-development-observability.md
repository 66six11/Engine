...
状态：Current R0.5 Baseline（protocol、shared-ring投影、Debug-only Host、current-user产品endpoint/discovery、typed CLI、壳 UI Probe 及现代stdio只读 MCP adapter 的 Slice 1→8 与2.11→2.13纠错格均已关闭）
...
### 2.10 R0.5 Slice 8 modern read-only stdio MCP adapter card（closed evidence）
...
| Official MCP precedent adopted | MCP官方[`2026-07-28` Discovery](https://modelcontextprotocol.io/specification/2026-07-28/server/discover)要求server实现`server/discover`并返回supported versions/capabilities；[Versioning](https://modelcontextprotocol.io/specification/2026-07-28/basic/versioning)取消negotiation handshake，要求每个request携protocol version；[stdio](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/stdio)规定newline-delimited UTF-8 JSON-RPC、stdout只能写协议消息、`notifications/cancelled`与stdin EOF shutdown；[Tools](https://modelcontextprotocol.io/specification/2026-07-28/server/tools)要求固定name/input schema、deterministic list、tool execution error与structured content。Asharia逐项采用。 |
| Rejected / Asharia rationale | 拒绝`2025-11-25`及更早`initialize`兼容、HTTP/SSE/remote transport、MCP resources/prompts/sampling/elicitation/tasks/subscription、server-to-client request、generic method/tool、state/readElement/find空壳、Capture/Mutate与CLI subprocess。官方C# SDK 2.x在本Slice时仍为RC/preview且引入不需要的hosting/DI面；tools-only协议可用BCL明确有界完成，因此不增加该依赖，待出现真实SDK互操作缺口再评审。 |
...
| Data / security / error | 每个request必须携`io.modelcontextprotocol/protocolVersion=2026-07-28`与clientCapabilities；缺失/重复field、非integer/null/duplicate ID、unknown method/tool、legacy initialize、版本不匹配、invalid args、invalid UTF-8/JSON/depth/size均fail closed。tools/list固定顺序且每项`readOnlyHint=true/destructiveHint=false/openWorldHint=false`。list只输出safe manifest摘要；tool result把typed protocol response放入structured content并保留partial/cursor/drop/truncation，attach token/pipe name不进入schema、prompt、result或stderr。 |
| Success / failure / timeout / cancel / shutdown | MCP focused 7/7覆盖modern discover/tool-list exact golden、legacy/missing metadata/unsupported/unknown/oversize/invalid argument、真实产品endpoint六tools与partial/drop/UI/secret、真实tool deadline typed timeout、duplicate request ID与第9个request Busy、cancel notification后无late response、stdin EOF下in-flight adapter取消并clean process exit。Pipe v1每connection严格1 in-flight且不会在provider await期间从断连即时传播取消；因此本Slice只声明adapter停止等待/释放连接，Host按既有deadline/owner stop有界drain，不伪称provider强取消。 |
...
| Non-goals | legacy MCP compatibility、HTTP/remote MCP、state/readElement/find、resources/prompts/tasks/sampling/elicitation、generic RPC、automatic target selection、watch/tail daemon、screenshot/input/property/style/object read、Capture/Mutate、metrics/trace/profiler/crash/artifact/test framework。 |
...
| Official MCP precedent adopted | MCP官方[`Cancellation`](https://modelcontextprotocol.io/specification/2026-07-28/basic/patterns/cancellation)要求receiver在request仍进行时停止工作、释放资源且不发送response，同时允许取消与完成竞争。Asharia把“取得writer gate并完成最后一次request-cancel检查”定义为frame commit点：commit前取消抑制整帧，commit后使用process lifetime token写完newline-delimited JSON，避免半帧破坏stdio。 |
...
| Non-goals | Pipe provider即时强取消、legacy initialize、HTTP/remote MCP、generic RPC、自动目标选择、Capture/Mutate、screenshot/input/property写、metrics/trace/profiler/crash/artifact/test framework。 |
...
| Earliest / latest gate | 这是 R0/R0.5 提交前验证审查的最后纠错格，严格晚于 2.14 的 typed client retention gate；必须在提交前完成，任何后续 R1 capability 仍需重新从 I0 → I6 进入。 |
...
当前只实现现代MCP `2026-07-28`：无`initialize`session，每个request携version/client capabilities；必须支持
`server/discover`，stdout只输出newline-delimited UTF-8 JSON-RPC，stdin EOF为graceful shutdown。旧版兼容、HTTP与双era fallback不属于本目标。

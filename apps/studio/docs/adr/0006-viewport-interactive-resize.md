...
V1–V6 stream symbols 不导出，managed 没有 fallback。兼容性 query、runtime stats 和 shutdown 是独立控制面，
不构成旧 frame path。
...
wrapper，再调用 `release_slot_import_v7`；native 只有在 import release 和 GPU completion 都成立后才销毁 slot。
...
`logical extent` 与 `allocation extent` 都等于这个物理像素尺寸；V7 ABI 仍保留两个字段，但 Studio presentation 不使用
...
- managed tests 覆盖：V7 ABI layout、view-local FOV axis、authored-mesh deep copy、自描述 ready frame/mesh receipt、exact-once completion、persistent slot identity、stream close；
...
- `--smoke-viewport-transaction-flash` 另以 typed V7 diagnostic flag 在 native Scene surface 写四色 corner sentinel，逐 batch 检查
...
- distribution 要求全部 V7 frame exports；V1–V6 stream exports 缺失是预期硬切结果。

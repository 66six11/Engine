# ADR-0006：Viewport Presentation Transaction 与 V6 多槽呈现
...
wrapper，再调用 `release_slot_import_v6`；native 只有在 import release 和 GPU completion 都成立后才销毁 slot。
...
`logical extent` 与 `allocation extent` 都等于这个物理像素尺寸；V6 ABI 仍保留两个字段，但 Studio presentation 不使用
...
- managed tests 覆盖：V6 ABI layout、authored-mesh deep copy、自描述 ready frame/mesh receipt、exact-once completion、persistent slot identity、stream close；
...
- `--smoke-viewport-transaction-flash` 另以 typed V6 diagnostic flag 在 native Scene surface 写四色 corner sentinel，逐 batch 检查
...
- distribution 要求全部 V6 frame exports；V5 frame exports 与旧 V4 create/release exports 缺失是预期硬切结果。

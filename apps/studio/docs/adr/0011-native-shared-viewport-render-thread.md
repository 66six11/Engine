# ADR-0011：Studio shared viewport 的唯一 RenderThread 与 V8 stream scheduler

状态：Accepted / Implemented
日期：2026-08-08

最近修订：2026-09-03（Viewport V8 typed Translate Gizmo packet 硬切）
...
V8 保留非阻塞 latest submit、非阻塞 ready take、显式 frame completion、显式 import release 和异步 close/poll/destroy，
并把 view-local FOV axis、authored mesh snapshots、per-view raster mode、request-correlated Scene mesh receipt 与
optional typed Translate Gizmo 纳入同一 owning packet。
V1–V7 frame symbols 不导出；历史 `query_runtime_stats_v2..v8` 是独立 diagnostics 版本链，不属于 stream protocol compatibility。
...
`close_stream_v8` 停止 admission、丢弃 pending latest，并把未 take 的 ready frame按 NotSubmitted 回收。Presented slot 必须先有
completion；曾暴露给 managed 的 slot 还必须收到 `release_slot_import_v8`。满足后 owner 调用现有 consumer-safe
...
- V8 目前只支持 `DocumentScene` target kind 和 Vulkan opaque NT handles；已知 validation mesh 可解析为真实 draw，未知/未就绪
  authored asset 逐项 no-draw，不会替换成默认模型。

## 验证

- MSVC native V8 smoke 验证 burst submit 的 latest-wins、coalesced counter、三个 distinct slots、reuse、view-local FOV axis，以及 authored mesh
...
- deterministic scheduler probe 验证四条 render lane 按稳定 ID 轮转、持续 realtime lane 不独占，以及两步 close 与 completion
  全部先于 render；真实 V8/Vulkan smoke 验证 cap=4 下四个 cold stream 都取得首帧，ready+pending realtime stream 不消耗其余
  cold first-slot 容量；
...
- DLL export audit 验证 V8 frame exports 存在、V1–V7 frame exports 不存在。

...
namespace asharia::archive {
    namespace {
        ...
        [[nodiscard]] Error jsonArchiveError(std::string message) {
            return Error{ErrorDomain::Archive, 0, std::move(message)};
        }
        ...
        [[nodiscard]] Result<ArchiveValue> fromJson(const OrderedJson& json) {
            ...
            if (json.is_number_unsigned()) {
                ...
                if (value > static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max())) {
                    return std::unexpected{jsonArchiveError(
            ...
            return std::unexpected{jsonArchiveError("Unsupported JSON value in archive.")};
        }
    ...
    Result<std::string> writeJsonArchive(const ArchiveValue& value) {
        try {
            std::string output =
            ...
            return std::unexpected{jsonArchiveError(exception.what())};
        } catch (const nlohmann::json::exception& exception) {
            return std::unexpected{
                jsonArchiveError("Failed to write JSON archive: " + std::string{exception.what()})};
    ...
    VoidResult writeJsonArchiveFile(const std::filesystem::path& path, const ArchiveValue& value) {
        auto text = writeJsonArchive(value);
        if (!text) {
            return std::unexpected{jsonArchiveError("Failed to serialize archive file '" +
    ...
    Result<ArchiveValue> readJsonArchive(std::string_view text) {
        try {
            ...
            return std::unexpected{jsonArchiveError("Failed to parse JSON archive at byte " +
                                                    std::to_string(exception.byte) + ": " +
                                                    exception.what())};
        } catch (const nlohmann::json::exception& exception) {
            return std::unexpected{
                jsonArchiveError("Failed to read JSON archive: " + std::string{exception.what()})};
        }
    }

...
BasicTriangleRenderer& BasicTriangleRenderer::operator=(BasicTriangleRenderer&& other) noexcept {
    if (this == &other) {
        return *this;
    }
...
Result<BasicTriangleRenderer> BasicTriangleRenderer::create(const BasicTriangleRendererDesc& desc) {
    ...
    if (desc.allocator == nullptr) {
        return std::unexpected{
            Error{ErrorDomain::Vulkan, 0, "Cannot create triangle renderer without an allocator"}};
    }
    const BasicDrawItem drawItem =
        desc.meshKind == BasicMeshKind::IndexedQuad ? basicIndexedQuadDrawItem() : desc.drawItem;
    if ((drawItem.vertexCount == 0 && drawItem.indexCount == 0) || drawItem.instanceCount == 0) {
        return std::unexpected{

...
BasicDrawListRenderer& BasicDrawListRenderer::operator=(BasicDrawListRenderer&& other) noexcept {
    if (this == &other) {
        return *this;
    }
...
Result<BasicDrawListRenderer> BasicDrawListRenderer::create(const BasicDrawListRendererDesc& desc) {
    ...
    if (desc.allocator == nullptr) {
        return std::unexpected{
            Error{ErrorDomain::Vulkan, 0, "Cannot create draw list renderer without an allocator"}};
    }
    ...
    const std::span<const BasicDrawListItem> drawItems =
        desc.drawItems.empty()
            ? std::span<const BasicDrawListItem>{defaultDrawItems.data(), defaultDrawItems.size()}
            : desc.drawItems;
...
BasicDrawListRenderer::recordFrame(const VulkanFrameRecordContext& frame) {
    ...
    auto recorded = compileExecuteAndFinalizeBasicGraph(frame, device_, allocator_, graph, bindings,

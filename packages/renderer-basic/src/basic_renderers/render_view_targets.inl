...
[[nodiscard]] BasicRenderViewTarget basicSampledRenderViewTarget(VulkanSampledTextureView target) {
    return BasicRenderViewTarget{
        .image = target.image,
...
[[nodiscard]] Result<void> validateBasicRenderViewTarget(const BasicRenderViewTarget& target,
...
[[nodiscard]] Result<RenderGraphImageDesc> basicRenderViewTargetDesc(
    const BasicRenderViewTarget& target, RenderGraphImageState initialState, std::string_view name,
    RenderGraphShaderStage initialShaderStage = RenderGraphShaderStage::None) {
    auto format = basicRenderGraphImageFormat(target.format, name);
    if (!format) {
...
basicRenderViewTargetBinding(RenderGraphImageHandle image, const BasicRenderViewTarget& target) {
